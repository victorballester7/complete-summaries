...
\begin{tikzpicture}[every node/.style={scale=0.7}]
  \begin{axis}[
    ...
    % \draw[draw=black,fill=black,name path=a] ({\r*(\T-sin(deg(\T))},{\r(1-cos(deg(\T)))}) circle(1.5pt) node[anchor=west]{$\vf{\alpha}(s_0)$};
    % \draw[draw=black,fill=black,name path=b] ({\r*(\T+sin(deg(\T)))},{\r*(-1+cos(deg(\T)))}) circle(1.5pt) node[anchor=west]{$C(s_0)$};
    \draw[<->] (a) -- (b) node[pos=0.65,anchor=north east]{$R(s_0)$};
  \end{axis}
\end{tikzpicture}
\end{document}