...
\begin{multicols}{2}[\section{Differential geometry}]
  ...
  \end{definition}
  \begin{definition}
    Let $U\subseteq\RR^n$ be an open set, $f\in\mathcal{C}^\infty(U)$ and $\vf{X}=\sum X^i\pdv{}{x^i}\in\mathcal{X}$. We denote by $\vf{X}f$ the differentiable function defined as: $$\vf{X}f(p):=\vf{X}_pf:=\sum_{i=1}^n X^i(p)\pdv{f}{x^i}\footnote{Observe that $\vf{X}_pf$ is the partial derivative of $f$ at $p$ in the direction of $\vf{X}_p$.}$$
  \end{definition}
  \begin{lemma}
  ...
  \end{definition}
  \begin{definition}
    Let $V$ be a vector space of dimension $n$ and $$\omega:V\times\overset{(k)}{\cdots}\times V\longrightarrow \RR$$ be a $k$-linear map. We say that $\omega$ is \emph{antisymmetric} if $$\omega(\vf{u}_{\sigma(1)},\ldots,\vf{u}_{\sigma(k)})=\sign(\sigma)\omega(\vf{u}_1,\ldots,\vf{u}_k)\qquad\forall\sigma\in \text{S}_k$$
    We denote by $\Lambda^kV^*$ the vector space of the antisymmetric $k$-linear maps. The elements of $\Lambda^kV^*$ are called \emph{multilinear forms}\footnote{Here $V^*$ denotes the dual space of $V$ (see \cref{LA_dual}).}. By agreement we denote $\Lambda^0V^*:=\RR$ and: $$\Lambda^*V^*:=\bigoplus_{k=0}^{n}\Lambda^kV^*$$
  \end{definition}
  \begin{definition}
  ...
  \end{proposition}
  \begin{definition}
    Let $U\subseteq\RR^n$ be an open set and $\omega\in \Omega^k(U)$. This $\omega$ defines a $\mathcal{C}^\infty(U)$-multilinear antisymmetric function, which we'll denote also by $\omega$, given by: $$\function{\omega}{\mathcal{X}(U)\times\overset{(k)}{\cdots}\times\mathcal{X}(U)}{\mathcal{C}^\infty(U)}{(\vf{X}_1,\ldots,\vf{X}_k)}{\omega(\vf{X}_1,\ldots,\vf{X}_k)}$$ where $\omega(\vf{X}_1,\ldots,\vf{X}_k)$ is the function defined by $\omega(\vf{X}_1,\ldots,\vf{X}_k)(p)=\omega_p({(\vf{X}_1)}_p,\ldots,{(X_k)}_p)$.
  \end{definition}
  \begin{proposition}
    Let $U\subseteq\RR^n$ be an open set and $h\in\mathcal{C}^\infty$. We define it \emph{differential} as the differential 1-form $\dd{h}$ given by $$\dd{h}=\sum_{i=1}^n\pdv{h}{x^i}\dd{x^i}$$
  \end{proposition}
  \begin{definition}
  ...
  \end{definition}
  \begin{proposition}
    Let $U\subseteq\RR^n$, $V\subseteq\RR^m$, $W\subseteq\RR^r$ be open sets, $\vf{f}:U\rightarrow V$, $\vf{g}:V\rightarrow W$ be a differentiable functions, $\omega,\eta\in\Omega^k(V)$, $h\in\Omega^0(V)$ and $a,b\in\RR$. Then:
    \begin{enumerate}
  ...
  \begin{corollary}
    ...
    \begin{enumerate}
      ...
      \item If $\omega=\sum_{j_1<\cdots<j_k}\omega_{j_1,\ldots,j_k}\dd{y^{j_1}}\wedge\cdots\wedge\dd{y^{j_k}}$, then: $$\vf{f^*}(\omega)=\sum_{j_1<\cdots<j_k}(\omega_{j_1,\ldots,j_k}\circ f)\dd{f^{j_1}}\wedge\cdots\wedge\dd{f^{j_k}}$$
    \end{enumerate}
  \end{corollary}
  \begin{definition}
    Let $U\subseteq\RR^n$ be an open set, $\vf{X}\in\mathcal{X}(U)$ and $\omega\in\Omega^k(U)$, where $k\geq 1$.
    We define the \emph{interior contraction} of $\omega$ by $\vf{X}$ as the differential $(k-1)$-form $i_{\vf{X}}\omega\in\Omega^{k-1}(U)$ defined by:
    $$i_{\vf{X}}\omega(\vf{Y}_2,\ldots,\vf{Y}_k)=\omega(\vf{X},\vf{Y}_2,\ldots,\vf{Y}_k)$$
    By agreement, we define $i_{\vf{X}}h=0$ if $h\in\Omega^0(U)$.
  \end{definition}
  \begin{proposition}
    Let $U\subseteq\RR^n$ be an open set, $\vf{X}\in\mathcal{X}(U)$, $\alpha\in\Omega^k(U)$, and $\beta\in\Omega^m(U)$. Then: $$i_{\vf{X}}(\alpha\wedge\beta)=(i_{\vf{X}}\alpha)\wedge\beta+{(-1)}^k\alpha\wedge(i_{\vf{X}}\beta)$$
  \end{proposition}
  \begin{definition}
  ...
  \end{definition}
  \begin{proposition}
    Let $(\vf{u}_1,\ldots,\vf{u}_n)$ be an orthonormal positive basis of $T_p\RR^n$, $p\in\RR^n$, and let $\eta$ be the element of volume. Then: $$\eta_p(\vf{u}_1,\ldots,\vf{u}_n)=1$$
  \end{proposition}
  \begin{proposition}
    Let $U,V\subseteq\RR^n$ be open sets and $\vf{f}:U\rightarrow V$ be a differentiable function. Then: $$\vf{f^*}\eta=J\vf{f}\eta$$ where $J\vf{f}$ is the Jacobian of $\vf{f}$.
  \end{proposition}
  \begin{definition}
