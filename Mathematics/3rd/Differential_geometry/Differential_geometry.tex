...
\begin{multicols}{2}[\section{Differential geometry}]
  ...
  \end{definition}
  \begin{definition}
    Let $U\subseteq\RR^n$ be an open set, $(\vf{e}_1,\ldots,\vf{e}_n)$ be the standard basis of $\RR^n$ and $\vf{X}$ be a vector field defined on $U$. For $i=1,\ldots,n$, we defined the following vector fields: $$\function{\vf{E}_i:=\pdv{}{x^i}}{\RR^n}{T_p\RR^n}{p}{(p,\vf{e}_i)}$$
  \end{definition}
  \begin{proposition}
  ...
  \end{definition}
  \begin{definition}
    Let $U\subseteq\RR^n$ be an open set, $f\in\mathcal{C}^\infty(U)$ and $\vf{X}=\sum X^i\pdv{}{x^i}\in\mathcal{X}$. We denote by $\vf{X}f$ the differentiable function defined as: $$\vf{X}f(p):=\vf{X}_pf:=\sum_{i=1}^n X^i(p)\pdv{f}{x^i}$$
  \end{definition}
  \begin{lemma}
  ...
  \end{lemma}
  \begin{lemma}
    Let $U\subseteq\RR^n$ be an open set, $f\in\mathcal{C}^\infty(U)$ and $\vf{X}\in\mathcal{X}$. Then: $$\vf{X}f=\langle\vf{X},\grad f\rangle$$
  \end{lemma}
  \begin{definition}
    Let $U\subseteq\RR^n$ be an open set and $\vf{X}=\sum X^i\pdv{}{x^i}\in\mathcal{X}(U)$. We say that a parametrized curve $\vf{\alpha}:I\rightarrow\RR^n$ is an \emph{integral curve} of $\vf{X}$ if: $$\vf\alpha'(t)=\vf{X}_{\vf\alpha(t)}\qquad \forall t\in I$$
  ...
  \end{definition}
  \begin{proposition}
    Let $U\subseteq\RR^n$ be an open set, $F\in\mathcal{C}^\infty(U)$ and $\vf{X}\in\mathcal{X}$. We say that $F$ if a \emph{first integral} of $\vf{X}$ if:
    \begin{enumerate}
      \item $\vf{\dd}F_p\ne 0$ $\forall p\in U$
  ...
  \end{proposition}
  \begin{definition}
    Let $S\subseteq\RR^3$ be a regular surface. A \emph{vector field} defined on $S$ is a correspondence $\vf{X}$ that at each point $p\in S$ it assigns a vector $\vf{X}(p)=:\vf{X}_p\in T_p\RR^3$. If there is a parametrization $\vf\varphi(u,v)$ of $S$, then we can write
    $$\vf{X}=\vf{X}(u,v)=\sum_{i=1}^3X^i(u,v)\pdv{}{x^i}$$
    where $X^i(u,v):=(X^i\circ\vf\varphi)(u,v)$. We say that $\vf{X}$ is \emph{differentiable} if it the functions $X^i(u,v)$ are differentiable. We say that $\vf{X}$ is \emph{tangent} to $S$ if $\vf{X}_p\in T_pS$ $\forall p\in S$. In this case we can write $$\vf{X}=X^1\vf\varphi_u+X^2\vf\varphi_v$$
  \end{definition}
  \begin{proposition}
  ...
  \end{proposition}
  \begin{corollary}
    Let $S\subseteq\RR^3$ be a regular surface and $p\in S$. Then, there exists an orthogonal parametrization $(V,\vf\varphi(u,v))$ of $S$ such that $p\in\vf\varphi(V)$.
  \end{corollary}
  ...
  \end{definition}
  \begin{definition}
    Let $V$ be a vector space and $$\omega:V\times\overset{(k)}{\cdots}\times V\longrightarrow \RR$$ be a $k$-linear map. We say that $\omega$ is \emph{antisymmetric} if $$\omega(\vf{u}_{\sigma(1)},\ldots,\vf{u}_{\sigma(k)})=\sign(\sigma)\omega(\vf{u}_1,\ldots,\vf{u}_k)\qquad\forall\sigma\in \text{S}_k$$
    We denote by $\Lambda^kV^*$ the vector space of the antisymmetric $k$-linear maps. The elements of $\Lambda^kV^*$ are called \emph{multilinear forms}\footnote{Here $V^*$ denotes the dual space of $V$ (see \cref{LA_dual})}. By agreement we denote $\Lambda^0V^*:=\RR$ and: $$\Lambda^*V^*:=\bigoplus_{k=0}^\infty\Lambda^kV^*$$
  \end{definition}
  \begin{definition}
  ...
  \begin{proposition}
    ...
    \begin{enumerate}
      \item $\alpha\wedge\beta\in\Lambda^{p+q}V^*$ (that is, $\alpha\wedge\beta$ is antisymmetric)
      \item $\alpha\wedge(\beta\wedge\gamma)=(\alpha\wedge\beta)\wedge\gamma$
      \item $\alpha\wedge\beta={(-1)}^{pq}\beta\wedge\gamma$
    \end{enumerate}
  \end{proposition}
  \begin{proposition}
  ...
  \begin{proposition}
    ...
    where $A_{j_1,\ldots,j_k}:={\vf{e}_1}^*\wedge\cdots\wedge{\vf{e}_k}^*(\vf{u}_1,\ldots,\vf{u}_k)$. Thus: $$\omega=\sum_{j_1<\cdots<j_k}\omega(\vf{e}_{j_1},\ldots,\vf{e}_{j_k}){\vf{e}_1}^*\wedge\cdots\wedge{\vf{e}_k}^*$$
  \end{proposition}
  \begin{corollary}
    Let $(\vf{e}_1,\ldots,\vf{e}_n)$ be the standard basis of $E:=\RR^n$ and $({\vf{e}_1}^*,\ldots,{\vf{e}_n}^*)$ be its associated dual basis. Then, the set $$\{{\vf{e}_1}^*\wedge\cdots\wedge{\vf{e}_k}^*:j_1<\cdots<j_k\}$$
    is a basis of $\Lambda^kE^*$. In particular, $\dim \Lambda^kE^*)\binom{n}{k}$.
  \end{corollary}
  \begin{corollary}
    Let $(\vf{e}_1,\ldots,\vf{e}_n)$ and $(\vf{v}_1,\ldots,\vf{v}_n)$ be the standard basis and another basis of $E:=\RR^n$ respectively with associated dual basis $({\vf{e}_1}^*,\ldots,{\vf{e}_n}^*)$ and $({\vf{v}_1}^*,\ldots,{\vf{v}_n}^*)$. Suppose $\vf{v}_i=\sum_{j=1}^na_{ij}\vf{e}_i$ for $i=1,\ldots,n$ and define $\vf{A}:=(a_{ij})$. Then: $${\vf{v}_1}^*\wedge\cdots\wedge{\vf{v}_n}^*=\frac{1}{\det\vf{A}}{\vf{e}_1}^*\wedge\cdots\wedge{\vf{e}_n}^*$$
  \end{corollary}
  \begin{proposition}
  ...
  \begin{definition}
    Let $U\subseteq\RR^n$ be an open set. A \emph{differential $k$-form} on $U$ is a differentiable function $\omega_U\rightarrow\Lambda^k{(\RR^n)}^*\cong\RR^{\binom{n}{k}}$\footnote{An element $\omega(p)=:\omega_p$ can be though as an element of $\Lambda^k{(T_p\RR^n)}^*$.}. We will denote dual basis of $\left(\pdv{}{x^1},\ldots,\pdv{}{x^n}\right)$ by $(\dd{x^1},\ldots,\dd{x^n})$. Thus, a differential $k$-form can be written as:
    $$\omega=\sum_{j_1<\ldots<j_k}\omega_{j_1,\ldots,j_k}\dd{x^{j_1}}\wedge\cdots\wedge\dd{x^{j_k}}$$
    We denote by $\Omega^k(U)$ the set of all the differential $k$-forms defined on $U$ with the agreement that $\Omega^0(U):=\mathcal{C}^\infty(U)$.
  \end{definition}
  \begin{proposition}
