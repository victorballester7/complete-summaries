...
\begin{multicols}{2}[\section{Statistics}]
  ...
  \begin{definition}[Neymann-Pearson test]
    Let $(\mathcal{X},\mathcal{F},\{\Prob_\theta^{\vf{X}_n}:\theta\in\Theta=\Theta_0\sqcup\Theta_1\})$ be a statistical model, $\vf{x}\in\mathcal{X}$ be a realization of $\vf{X}_n$, $\delta:\mathcal{X}=A_1\sqcup A_2\rightarrow\{\mathcal{H}_0,\mathcal{H}_1\}$ be a hypothesis test such that $\Theta_0=\{\theta_0\}$ and $\Theta_1=\{\theta_1\}$. We say that $\delta$ is a \emph{Neyman-Pearson test} (or \emph{Likelihood-ratio test}) of significance level $\alpha\in[0,1]$ if $\exists C>0$ such that:
    \begin{gather*}
