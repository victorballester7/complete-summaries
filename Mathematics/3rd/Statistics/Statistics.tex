...
\begin{multicols}{2}[\section{Statistics}]
  ...
  \begin{definition}[Quantile]\label{STA_quantile}
    We define quantile function $Q_X(p)$ of a distribution of a random variable $X$ as the inverse function of the cmf. That is, $Q_X(p)$ satisfies:
    $$\Prob(X\leq Q_X(p))=p$$
  ...
  \begin{definition}[Test of independence]
    ...
    $$
      ...
      \end{cases}
    $$
    Again, the distribution of the data in the table follows a multinomial distribution, so under $\mathcal{H}_0$ we get the following MLEs for $\theta_i$ and $\phi_j$ (with the constraints that $\sum_{i=1}^r\theta_i=\sum_{j=1}^s\theta_j=1$):
    $$\hat\theta_i=\frac{n_{i\cdot}}{n}\quad\hat\phi_j=\frac{n_{\cdot j}}{n}\qquad\forall i,j$$
    And in general, using the constriant $\sum_{i=1}^r\sum_{j=1}^sp_{ij}=1$, we have: $$\hat{p}_{ij}=\frac{n_{ij}}{n}\qquad\forall i,j$$
