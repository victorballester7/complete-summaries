...
\begin{multicols}{2}[\section{Topology}]
  ...
  \end{prop}
  \begin{prop}
    Let $(X_i,\mathcal{T}_{X_i})$ and $(Y_i,\mathcal{T}_{Y_i})$ be topological spaces, $X:=\prod_{i=1}^nX_i$, $Y:=\prod_{i=1}^nY_i$ and $f_i:(X_i,\mathcal{T}_{X_i})\rightarrow(Y_i,\mathcal{T}_{Y_i})$ be a continuous function for $i=1,\ldots,n$. Then,
    $$
      \function{f_1\times\cdots\times f_n}{\left(X,\mathcal{T}_X\right)}{\left(Y,\mathcal{T}_Y\right)}{(x_1,\ldots,x_n)}{(f(x_1),\ldots,f(x_n))}
    $$ is also continuous.
  \end{prop}
  \begin{prop}
