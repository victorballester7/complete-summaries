...
\begin{multicols}{2}[\section{Differential equations}]
  ...
  \begin{theorem}[Dependence on $\vectorfunction{x}_0$]
  ...
  \begin{theorem}[Dependence on $t_0$]
    ...
    \end{equation*}
  \end{theorem}
  \begin{theorem}[Dependence on $\vectorfunction{\lambda}$]
  ...
  \end{definition}
  \begin{definition}
    Let $(G,X,\Psi)$ be a dynamical system with $x\in X$. The \textit{orbit} through $x$ is defined as: $$\gamma(x):=\{\Psi(t,x):t\in G\}$$ Moreover if $G=\ZZ$ or $G=\RR$ we define the \textit{positive semi-orbit} through $x$ and the \textit{negative semi-orbit} through $x$ as the following respective sets:
    \begin{gather*}
