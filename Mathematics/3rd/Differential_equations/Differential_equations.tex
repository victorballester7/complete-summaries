...
\begin{multicols}{2}[\section{Differential equations}]
  ...
  \begin{definition}
    ...
    The set of all $\alpha$-limit points of $\vf{x}$ is called \emph{$\alpha$-limit set}, and it is denoted by $\alpha(\vf{x})$. For an orbit $\vf{\gamma}$ of $(\RR,\RR^n,\vf{\Psi})$, we say that $\vf{y}$ is an \emph{$\omega$-limit point} of $\vf{\gamma}$, it is a $\omega$-limit point of some point on the orbit $\vf{\gamma}$.
  \end{definition}
  \begin{definition}
    ...
    The set of all $\omega$-limit points of $\vf{x}$ is called \emph{$\omega$-limit set}, and it is denoted by $\omega(\vf{x})$. For an orbit $\vf{\gamma}$ of $(\RR,\RR^n,\vf{\Psi})$, we say that $\vf{y}$ is an \emph{$\omega$-limit point} of $\vf{\gamma}$, it is a $\omega$-limit point of some point on the orbit $\vf{\gamma}$.
  \end{definition}
  \begin{proposition}
  ...
  \end{definition}
  \begin{proposition}
    Let $(\RR,\RR^n,\vf{\Psi})$ be a dynamical system and $\vf{p}\in \RR^n$. Suppose $\vf{\gamma}^+$ is contained in a compact set. Then:
    \begin{itemize}
      \item $\omega(\vf{p})\ne\varnothing$.
  ...
  \end{definition}
  \begin{proposition}
    Let $U\subseteq \RR^{2n}$ be an open set, $H:U\rightarrow\RR$ be the Hamiltonian of its Hamiltonian system. Then, $H$ is a first integral of that differential system.
  \end{proposition}
  ...
  \begin{proposition}
    ...
    \begin{itemize}
      ...
      \item $\vf{\gamma}$ is \emph{stable} if $\omega(\vf{q})=\vf{\gamma}$ $\forall \vf{q}\in V$.
      \item $\vf{\gamma}$ is \emph{semi-stable} if either $\omega(\vf{q})=\vf{\gamma}$ $\forall \vf{q}\in V\cap\Ext(\vf{\gamma})$ and $\alpha(\vf{q})=\vf{\gamma}$ $\forall \vf{q}\in V\cap\Int(\vf{\gamma})$, or $\omega(\vf{q})=\vf{\gamma}$ $\forall \vf{q}\in V\cap\Int(\vf{\gamma})$ and $\alpha(\vf{q})=\vf{\gamma}$ $\forall \vf{q}\in V\cap\Ext(\vf{\gamma})$.
    \end{itemize}
  \end{proposition}
  \subsubsection{Integrability of polynomial vectors fields}
  \begin{definition}
  ...
  \end{definition}
  \begin{definition}
    Let $f,p,q\in\RR[x,y]$ and consider the polynomial system of degree $n$ of \cref{DE_poly}. We say that $f(x,y)=0$ is \emph{algebraic invariant curve} under the system of \cref{DE_poly} if $$\pdv{f}{x}(x,y)p(x,y)+\pdv{f}{y}q(x,y)=k(x,y)f(x,y)$$
    where $k\in\RR[x,y]$ is called \emph{cofactor} of the invariant curve $f(x,y)=0$. Note that $\deg k\leq n-1$.
  \end{definition}
  \begin{proposition}
    Let $f,p,q\in\RR[x,y]$ and consider the polynomial system of degree $n$ of \cref{DE_poly}. Then, the invariant curve $f(x,y)=0$ is an orbit of the differential system of \cref{DE_poly}.
  \end{proposition}
\end{multicols}
\end{document}