...
\begin{multicols}{2}[\section{Differential equations}]
  ...
  \begin{definition}
    ...
    \begin{equation}\label{DE_poly}
      ...
      \right.
    \end{equation}
    is called a \emph{polynomial system}. The field $\vf{f}=(p,q)$ is called \emph{polynomial vector field}. We define the degree of that system as $n:=\max\{\deg p,\deg q\}$.
  \end{definition}
  \begin{definition}
  ...
  \end{definition}
  \begin{definition}
    Let $U,\tilde{U}\subseteq \RR^n$ be open sets with $\tilde{U}\subseteq U$ and $\vf{f}:U\rightarrow\RR^n$ be a vector field of class $\mathcal{C}^1$ and $\vf{p}\in \tilde{U}$ be a critical point of $\vf{f}$. We say that a function of class $\mathcal{C}^1$, $V:\tilde{U}\rightarrow\RR$, is a \emph{Lyapunov function for $\vf{p}$} if
    \begin{itemize}
  ...
  \begin{theorem}[Bendixson's theorem]
    Let $U\subseteq \RR^2$ be an open set and $\vf{f}:U\rightarrow\RR^2$ be a vector field of class $\mathcal{C}^1$  such that $\div \vf{f}$ has constant sign in a simply connected region and is not identically zero on any subregion of $R$ with positive measure. Then, the system $\vf{x}'=\vf{f}(\vf{x})$ does not have periodic orbits that lie entirely on $R$.
  \end{theorem}
  \begin{theorem}[Bendixson-Dulac theorem]
    Let $U\subseteq \RR^2$ be an open set and $\vf{f}:U\rightarrow\RR^2$ be a vector field of class $\mathcal{C}^1$ and suppose that there exists a function $h:\RR^2\rightarrow\RR$ such that $\div (h\vf{f})$ has constant sign in a simply connected region and is not identically zero on any subregion of $R$ with positive measure. Then, the system $\vf{x}'=\vf{f}(\vf{x})$ does not have periodic orbits that lie entirely on $R$.
  \end{theorem}
