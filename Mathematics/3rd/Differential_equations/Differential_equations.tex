...
\begin{multicols}{2}[\section{Differential equations}]
  ...
  \begin{proposition}
    Let $\gamma:I\rightarrow\RR^2$ be a closed path, $q\in \RR\setminus\gamma^*$ and $L$ be a ray with vertex at $q$. Consider the function $\varphi:I\rightarrow\RR$ that measures the oriented angle between $\gamma(t)$ and $L$. Then: $$\Ind(\gamma,q)=\frac{\varphi(1)-\varphi(0)}{2\pi}$$
  \end{proposition}
  \begin{proposition}
    Let $q_1,q_2\in\RR^2$ and $\gamma:I\rightarrow\RR^2$ be a closed path such that the segment $\overline{q_1q_2}$ does not intersect $\gamma^*$. Then, $\Ind(\gamma,q_1)=\Ind(\gamma,q_2)$
  \end{proposition}
  \begin{corollary}
    Let $\gamma:I\rightarrow\RR^2$ be a closed path. Then, all points in the same conencted component of $\RR^2\setminus\gamma^*$ have the same index.
  \end{corollary}
  \begin{proposition}
    Let $\gamma_1,\gamma_2:I\rightarrow\RR^2$ be two closed path and $q\in\RR^2$ be such that $q\notin\overline{\gamma_1(t)\gamma_2(t)}$ $\forall t\in I$. Then, $\Ind(\gamma_1,q)=\Ind(\gamma_2,q)$.
  \end{proposition}
  % \begin{proposition}[Poincaré index formula]
  %   Let $q$ be a singular point with a finite number of sectors. Denote by $e$ the number of elliptic sectors; by $h$, the number of hyperbolic sectors, and by $p$, the number of parabolic sectors. Then: $$\Ind(\gamma,q)=\frac{e-h}{2}+1$$
  % \end{proposition}
  \begin{theorem}[Poincaré-Hopf theorem]
    Consider a differential system defined on $S^2$ with a finite number of singular points. Then, the sum of their indices is 2.
  \end{theorem}
\end{multicols}
\end{document}