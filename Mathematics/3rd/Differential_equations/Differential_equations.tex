...
\begin{multicols}{2}[\section{Differential equations}]
  ...
  \end{proposition}
  \begin{definition}
    Let $U\subseteq S^2$ be an open set. A \emph{tangent vector field} defined on $S^2$ is a vector field $\vf{X}$ such that $\vf{X}(q)\in T_pS^2$ $\forall q\in U$.
  \end{definition}
  \begin{definition}
    Let $U\subseteq S^2$ be an open set and $\vf{X}:U\rightarrow\RR^3$ be a tangent vector field and $p$ be a singular point of $\vf{X}$. Suppose (rotating the sphere if necessary) that $p$ is on one of its poles. Let $\vf{\tilde{X}}$ be the field created from the stereographic projection from $-p$ to the equator plane. We define the index of $p$ with respect to the field $\vf{X}$ as: $$\Ind_p(X)=\Ind_0(\vf{\tilde{X}})$$
  \end{definition}
