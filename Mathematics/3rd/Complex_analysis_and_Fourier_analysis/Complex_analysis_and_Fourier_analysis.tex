...
\begin{multicols}{2}[\section{Complex analysis and Fourier analysis}]
  ...
  \begin{proposition}
    ...
    \begin{enumerate}
      ...
      \item $\abs{z\pm w}\leq \abs{z}+ \abs{w}$ and the equality holds if and only if $z=\lambda w$, $\lambda\in\RR_{\geq 0}$.
      \item $\abs{\abs{z}-\abs{w}}\leq \abs{z\pm w}$ and the equality holds if and only if $z=\lambda w$, $\lambda\in\RR_{\geq 0}$.
    \end{enumerate}
  \end{proposition}
  \begin{corollary}
  ...
  \end{definition}
  \begin{definition}
    Let $D\subseteq \CC^*$ be a set. A \emph{determination of the argument} on $D$ is a function $g:D\rightarrow \RR$ such that $g(z)\in\arg z$ $\forall z\in D$. That is, $g$ is the choice of a value in $\arg z$ for each $z\in D$.
  \end{definition}
  \begin{definition}
  ...
  \end{proposition}
  \begin{definition}
    Let $U\subseteq \CC$ be an open set, $f:U\rightarrow \CC$ be a function and $z_0\in U$. We say that $f$ is \emph{$\RR$-differentiable} at $z_0\in U$ if and only if there exists a $\RR$-linear function $L:\CC\rightarrow\CC$ such that: $$\lim_{h+k\ii\to 0}\frac{|f(z_0+h+k\ii)-f(z_0)-L(h+k\ii)|}{|h+k\ii|}=0$$
    In that case, the function $L$ is called \emph{differential} of $f$ at $z_0$ and it is denoted by $\vf{D}f(z_0)$\footnote{That is, the $\RR$-differentiability is the usual one if we think $f$ inside $\RR^2$ instead of inside $\CC$.}.
  \end{definition}
  \begin{proposition}
  ...
  \begin{definition}
    Let $U\subseteq \CC\cong\RR^2$ be an open set and $f:U\rightarrow \CC$ be a function of class $\mathcal{C}^2$. We say that $f$ is \emph{harmonic} if $\laplacian f(z)=0$ $\forall z\in U$\footnote{Recall \cref{FOSV_laplacian}.}.
  \end{definition}
  \begin{proposition}
