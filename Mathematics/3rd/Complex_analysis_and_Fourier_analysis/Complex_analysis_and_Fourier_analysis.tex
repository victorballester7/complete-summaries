...
\begin{multicols}{2}[\section{Complex analysis and Fourier analysis}]
  ...
  \begin{theorem}[Mean value property]
    Let $U\subseteq\CC$, $u:U\rightarrow\CC$ be a harmonic function, $a\in\CC$ and $r\in\RR_{>0}$. Then:
  ...
  \end{corollary}
  \subsubsection{Some important consequences}
  \begin{theorem}
  ...
  \end{theorem}
  \begin{definition}
    Let $U\subseteq\CC$ be an open connected set. We say that $U$ is simply connected if $\CC_\infty\setminus U$ is connected.
  \end{definition}
  \begin{proposition}
  ...
  \end{proposition}
  \begin{definition}
    Let $U\subseteq\CC$ be an open set. We say that $U$ is simply connected if each connected component is simply connected.
  \end{definition}
  \begin{theorem}
  ...
  \end{theorem}
  \subsubsection{Singularities of holomorphic functions}
  \begin{definition}
  ...
  \end{theorem}
  \begin{corollary}
    Let $U\subseteq \CC$ be an open set, $a\in U$, $f=\in\mathcal{H}(U\setminus\{a\})$ be a function with an isolated singularity at $a$. Then:
    \begin{enumerate}
      \item $a$ is a removable singularity $\iff a_n=0\ \forall n\leq -1$.
      \item $a$ is a pole of order $m\in\NN$ $\iff a_n=0\ \forall n\leq -m-1$ and $a_{-m}\ne 0$.
      \item $a$ is an essential singularity $\iff$ there exist infinite $n\in\NN$ such that $a_{-n}\ne 0$.
    \end{enumerate}
  \end{corollary}
  ...
  \end{theorem}
  \begin{proposition}
    Let $U\subseteq \CC$ be an open set, $a\in U$, $f\in\mathcal{H}(U\setminus\{a\})$ be a function with a pole of order $m$ at $a$. Then: $$\Res(f,a)=\frac{1}{(m-1)!}\left.\dv[m-1]{}{z}\left[{(z-a)}^mf(z)\right]\right|_{z=a}$$
  ...
  \begin{theorem}[Casorati-Weierstra\ss\space theorem]
    Let $a\in\CC$, $r\in\RR_{>0}$ and $f\in\mathcal{H}(D(a,r)\setminus\{a\})$ be such that it has an essential singularity at $a$. Then, $\forall \delta>0$: $$\overline{f(\{z\in\CC:0<\abs{z-a}<\delta\})}=\CC$$
  \end{theorem}
  \begin{corollary}
  ...
  \begin{theorem}[Great Picard's theorem]
    Let $a\in\CC$, $r\in\RR_{>0}$, $w\in\CC$ and $f\in\mathcal{H}(D(a,r)\setminus\{a\})$ be such that it has an essential singularity at $a$. Then, $\exists\alpha\in\CC$ such that $\forall \delta>0$: $$\overline{f(\{z\in\CC:0<\abs{z-a}<\delta\})}\supseteq\CC\setminus\{\alpha\}$$
  \end{theorem}
  \begin{definition}
    Let $U\subseteq \CC$ be an open set, $A\subset U$ such that $A'\cap U=\varnothing$, $f\in\mathcal{H}(U\setminus A)$ and for each $a\in A$, $f$ has a pole at $a$. In these conditions, we say that $f$ is \emph{meromorphic} and we denote the set of all meromorphic on $U$ by $\mathcal{M}(U)$.
  \end{definition}
  \begin{theorem}[Argument principle]
    Let $U\subseteq \CC$ be an open set and $f\in\mathcal{M}(U)$. We denote by $Z$ the set of zeros of $f$ and by $P$ the set of its poles. Let $\gamma\homoleg{U} 0$ be a piecewise cycle of class $\mathcal{C}^1$ such that $\gamma^*\subset U\setminus (Z\cup P)$. Then:
    $$\frac{1}{2\pi\ii}\int_{\gamma}\frac{f'(z)}{f(z)}\dd{z}=\sum_{z\in Z}\Ind(\gamma,z)-\sum_{p\in P}\Ind(\gamma,p)+$$
  \end{theorem}
  %\subsubsection{Applications of residues theorem}
  \begin{method}[Calculation of integrals]
    For the following cases use the function and curve given to find the integrals using the Residues theorem.
    \begin{enumerate}
      \item $\displaystyle\int_0^{2\pi} g(\cos x,\sin x)\dd{x}$, $g\in\RR(x)$.
            \begin{enumerate}
              \item Function: $f(z)=g\left(\frac{z+\frac{1}{z}}{2},\frac{z-\frac{1}{z}}{2\ii}\right)$
              \item Curve: $\gamma(t)=\exp{\ii t}$, $t\in[0,2\pi]$
            \end{enumerate}
      \item $\displaystyle\int_{-\infty}^{+\infty} g(x)\dd{x}$, $g=\frac{p}{q}\in\CC(x)$ such that it doesn't have singularities on $\RR$ and $\deg q\geq \deg p+2$.
            \begin{enumerate}
              \item Function: $f(z)=g(z)$
              \item Curve: $\gamma(t)=\gamma_1(t)+\gamma_2(t)$, where:
                    $$
                      \begin{cases}
      ...
      \item $\displaystyle\int_0^{+\infty} \frac{g(x)}{x^\alpha}\dd{x}$, $g\in\RR(x)$ and $\alpha\in\RR$ be such that the integral converges.
            \begin{enumerate}
              ...
              \item Curve: $\gamma(t)=\sum_{i=1}^4\gamma_i(t)$, where:
                    $$
                      \begin{cases}
