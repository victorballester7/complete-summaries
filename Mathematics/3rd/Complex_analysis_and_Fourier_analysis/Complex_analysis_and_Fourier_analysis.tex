...
\begin{multicols}{2}[\section{Complex analysis and Fourier analysis}]
  ...
  \begin{theorem}[Cauchy's integral theorem]
    Let $U\subseteq \CC$ be a connected open set, $f\in\mathcal{H}(U)$ and $\gamma:[a,b]\rightarrow\CC$ be a closed $\mathcal{C}^1$-piecewise curve. Then: $$\int_\gamma f(z)\dd{z}=0$$
  \end{theorem}
  \begin{lemma}
    Let $U\subseteq \CC$ be an open connected set, $z_0\in U$, $f\in\mathcal{H}(U\setminus\{p\})$ and $\gamma:[a,b]\rightarrow\CC$ be a closed $\mathcal{C}^1$-piecewise curve such that $\gamma^*\subseteq U\setminus\{z_0\}$. Suppose $\displaystyle\lim_{z\to z_0}f(z)(z-z_0)=0$. Then: $$\int_\gamma f(z)\dd{z}=0$$
  \end{lemma}
\end{multicols}
\end{document}