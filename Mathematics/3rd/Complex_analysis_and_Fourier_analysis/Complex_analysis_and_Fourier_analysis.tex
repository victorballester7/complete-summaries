...
\begin{multicols}{2}[\section{Complex analysis and Fourier analysis}]
  ...
  \end{theorem}
  \begin{corollary}
    Let $U\subseteq\CC$ be an open connected set, $f\in\mathcal{H}(U)$ be not identically zero and $Z(f)$ be the set of zeros of $f$. Then:
    \begin{enumerate}
      \item $\forall z\in Z(f)$, $z$ is isolated of $Z(f)$.
      \item $Z(f)$ is countable.
    \end{enumerate}
  \end{corollary}
  ...
  \begin{corollary}
    ...
    \begin{enumerate}
      ...
      \item If $f$ doesn't have zeros of $\overline{U}$, $\abs{f}$ attains its absolute minimum on $\Fr{U}$, that is: $$\min\{\abs{f(z)}:z\in\overline{U}\}=\min\{\abs{f(z)}:z\in\Fr U\}$$
    \end{enumerate}
  \end{corollary}
  ...
  \begin{lemma}[Schwarz lemma]
    Let $f\in\mathcal{H}(D(0,1))$ such that $f(0)=0$ and $\abs{f(z)}\leq 1$. Then:
    \begin{enumerate}
      \item $\abs{f'(0)}\leq 1$
      \item $\abs{f(z)}\leq z$ $\forall z\in D(0,1)$
    \end{enumerate}
    Moreover if $\exists w\in D(0,1)$ such that the equality holds in either of the inequalities of above, then $f(z)=\exp{\ii \theta} z$, for some $\theta\in\RR$.
  \end{lemma}
  \begin{definition}
    Let $a\in D(0,1)$. We define the Möbius transformation $\varphi_a$ as: $$\varphi_a(z)=\frac{z-a}{1-\overline{a}z}$$
  \end{definition}
  \begin{proposition}
    Let $a\in D(0,1)$. Then:
    \begin{enumerate}
      \item $\varphi_a(\overline{D(0,1)})=\overline{D(0,1)}$
      \item $\varphi_a(\Fr{D(0,1)})=\Fr{D(0,1)}$
    \end{enumerate}
  \end{proposition}
  \begin{theorem}
    Let $f:D(0,1)\rightarrow D(0,1)$ be a holomorphic and bijective function and $a\in D(0,1)$ such that $f(a)=0$. Then, $\exists\theta\in\RR$ such that $f(z)=\exp{\ii \theta} \varphi_a(z)$.
  \end{theorem}
  \begin{lemma}[Schwarz-Pick lemma]
    Let $f\in\mathcal{H}(D(0,1))$ such that $f(0)=0$ and $\abs{f(z)}\leq 1$. Then $\forall a,z\in D(0,1)$ we have:
