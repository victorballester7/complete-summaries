...
\begin{multicols}{2}[\section{Complex analysis and Fourier analysis}]
  ...
  \end{definition}
  \begin{definition}
    Let $\mathcal{F}\subseteq \mathcal{C}(U,\CC)$. We say that $\mathcal{F}$ is \emph{bounded by compact sets} if for each compact set $K\subset U$ $\exists M_K>0$ such that $\forall z\in K$ and all $f\in\mathcal{F}$ we have $\abs{f(z)}\leq M_K$.
  \end{definition}
  \begin{lemma}
