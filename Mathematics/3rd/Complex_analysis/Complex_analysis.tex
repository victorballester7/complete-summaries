...
\begin{multicols}{2}[\section{Complex analysis}]
  ...
  \begin{proposition}
    ...
    \begin{itemize}
      \item If the parametrization is positive\footnote{Recall \cref{DG_reparam}.}, then: $$\int_{\gamma_1}f(z)\dd{z}=\int_{\gamma_2}f(z)\dd{z}$$
  ...
  \begin{definition}
    Let $U\subseteq \CC\cong\RR^2$ be an open set and $u:U\rightarrow \RR$ be a function of class $\mathcal{C}^2$. We say that $u$ is \emph{harmonic} if $\laplacian u(z)=0$ $\forall z\in U$\footnote{Recall \cref{FSV_laplacian}.}.
  \end{definition}
  \begin{proposition}
