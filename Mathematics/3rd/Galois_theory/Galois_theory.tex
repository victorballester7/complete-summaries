...
\begin{multicols}{2}[\section{Galois theory}]
  ...
  \begin{theorem}[Universal property of the field of fractions]
    All integral domains are a subring of a field. More explicitly, if $R$ is an integral domain and $K$ is a field, there exists another field $Q(R)$\footnote{Recall \mcref{AS:field-frac} of \mcref{AS:R} for a formal definition of the field $Q(R)$.} and an injective ring morphism $\iota:R\hookrightarrow Q(R)$ so that for all injective ring morphism $f:R\hookrightarrow K$, there exists a unique field morphism $\psi_f:Q(R)\rightarrow K$ defined by
    $$
