...
\end{theorem}
\begin{theorem}[\bfseries Teorema de la P-A-Q reducció]
...
\end{theorem}
\begin{definition}
Sigui $\varphi$ una forma bilineal simètrica sobre $E$. Sigui $W$ un subespai vectorial de $E$. Definim l'ortogonal de $W$ respecte de $\varphi$ per $$W^\bot=\{u\in E\mid\varphi(u,\omega)=0\;\forall\omega\in W\}$$ Definim el radical de $\varphi$ per $$\text{rad }\varphi=E^\bot$$ Direm que $\varphi$ és no singular si $\text{rad}(\varphi)=\{0\}$.
\end{definition}
\begin{definition}
...
\begin{enumerate}
    \item $\dim E=\dim W+\dim W^\bot$.
    \item $(W^\bot)^\bot=W$.
    \item Si la restricció de $\varphi$ sobre $W$ és no singular, llavors $E=W\oplus W^\bot$.
\end{enumerate}
\end{definition}
\begin{definition}
Sigui $\varphi$ una forma bilineal simètrica sobre $E$. Direm que la suma $W_1+W_2$ de dos subespais vectorials $W_1$ i $W_2$ de $E$ és una suma ortogonal si és directa i $\varphi(u,v)=0$ $\forall u\in W_1$ i $v\in W_2$. Escriurem $W_1\bot W_2$ per denotar que la suma $W_1+W_2$ és ortogonal.
\end{definition}
\begin{definition}
Sigui $\varphi$ una forma bilineal simètrica sobre $E$. Siguin $W_1$ i $W_2$ dos subespais vectorials de $E$ tals que $E=W_1\bot W_2$. Llavors per a cada vector $v\in E$ existeixen $v_1\in W_1$ i $v_2\in W_2$ únics tals que $v=v_1+v_2$. L'aplicació $\pi:E\rightarrow W_i$ definida per $\pi(v)=v_i$ amb $v_i\in W_i$ es diu que és la projecció ortogonal de $E$ sobre $W_i$ segons la descomposició $E=W_1\bot W_2$.
\end{definition}
\begin{definition}
...
\end{definition}
\begin{definition}
Un producte escalar sobre un $\mathbb{R}$-espai vectorial $E$ és una forma bilineal simètrica definida positiva sobre $E$. Un espai vectorial euclidià és un parell $(E,\varphi)$, on $E$ és un $\mathbb{R}$-espai vectorial i $\varphi$ és un producte escalar sobre $E$.
\end{definition}
