...
\begin{multicols}{2}[\section{Mathematical analysis}]
  ...
  \end{proposition}
  \begin{definition}
    Let $f:(0,L)\rightarrow\CC $ be a function. We define the \emph{even extension} of $f$ as $$\Tilde{f}(x)=\left\{
      \begin{array}{rcl}
        f(x)  & \text{si} & x\in(0,L)  \\
        f(-x) & \text{si} & x\in(-L,0)
      \end{array}\right.$$ Analogously, we define the \emph{odd extension} of $f$ as $$\hat{f}(x)=\left\{
      \begin{array}{rcl}
        f(x)   & \text{si} & x\in(0,L)  \\
        -f(-x) & \text{si} & x\in(-L,0)
      \end{array}\right.$$
