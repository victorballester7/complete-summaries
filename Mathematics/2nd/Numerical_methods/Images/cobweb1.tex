...
\begin{document}
\pgfkeys{/pgf/declare function={f(\x)=cos(deg(\x));}} % Here you can declare multivariable functions to like this:
% declare function={
% excitation(\t,\w) = sin(\t*\w);
% noise = rnd - 0.5;
...
\begin{tikzpicture}
    ...
    \draw [thin,lessgreen,dashed] (axis cs:0.739,0) -- node[left,pos=0.05]{$\alpha$} (axis cs:0.739,0.739);
    \legend{$g(x)$}
    \end{axis}
\end{tikzpicture}
\end{document}