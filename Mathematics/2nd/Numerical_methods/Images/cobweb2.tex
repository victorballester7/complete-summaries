...
\begin{document}
\pgfkeys{/pgf/declare function={f(\x)=1.65*\x*(1-0.4*\x);}} % Here you can declare multivariable functions to like this:
% declare function={
% excitation(\t,\w) = sin(\t*\w);
% noise = rnd - 0.5;
...
\begin{tikzpicture}
    \begin{axis}[xmin=0,xmax=1.2,ymin=0,ymax=1.2,
        ...
        \draw [thin,color_green2,dashed] (axis cs:0.985,0) -- node[left,pos=0.05]{$\alpha$} (axis cs:0.985,0.985);
        \legend{$g(x)$}
    \end{axis}
\end{tikzpicture}
\end{document}