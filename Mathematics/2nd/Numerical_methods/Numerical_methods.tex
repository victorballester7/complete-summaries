...
\end{lemma}
\begin{definition}
    Let $\mathblack{A}\in\mathcal{M}_n(\CC)$ be a matrix with eigenvalues $\lambda_1,\ldots,\lambda_n$. We define the \textit{spectral radius $\rho(\mathblack{A})$} as: $$\rho(\mathblack{A})=\max\{|\lambda_1|,\ldots,|\lambda_n|\}.$$
\end{definition}
\begin{prop}
...
\end{equation*}
\begin{theorem}
    The iterative method $\mathblack{x}^{(k+1)}=\mathblack{M}\mathblack{x}^{(k)}+\mathblack{N}^{-1}\mathblack{b}$ is convergent if and only if $\mathblack{M}$ is convergent and if and only if $\rho(M)$.
\end{theorem}
\begin{corollary}
...
\end{corollary}
\begin{definition}
    We define the \textit{rate of convergence $R$} of an iterative method $\mathblack{x}^{(k+1)}=\mathblack{M}\mathblack{x}^{(k)}+\mathblack{N}^{-1}\mathblack{b}$ as: $$R=-\log(\rho(M)).$$
\end{definition}
\begin{prop}
...
\end{definition}
\begin{theorem}
    Let $\mathblack{A}=(a_{ij})\in\mathcal{M}_n(\RR)$ and $\mathblack{b}\in\RR^n$. If $\mathblack{A}$ is strictly diagonally dominant by rows or columns, then Jacobi method applied to solve the system $\mathblack{A}\mathblack{x}=\mathblack{b}$ is convergent.
\end{theorem}
...
\end{definition}
\begin{theorem}
    Let $\mathblack{A}=(a_{ij})\in\mathcal{M}_n(\RR)$ and $\mathblack{b}\in\RR^n$. If $\mathblack{A}$ is strictly diagonally dominant by rows, then Gau\ss-Seidel method applied to solve the system $\mathblack{A}\mathblack{x}=\mathblack{b}$ is convergent.
\end{theorem}
\end{multicols}
\end{document}