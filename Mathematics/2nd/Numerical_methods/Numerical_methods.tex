...
\end{definition}
\begin{center}
    \includegraphics[width=0.49\linewidth]{Images/cobweb1.tex}
    \includegraphics[width=0.49\linewidth]{Images/cobweb2.tex}\\
    \includegraphics[width=0.49\linewidth]{Images/cobweb3.tex}
...
\begin{theorem}[Composite Trapezoidal rule]
    ...
    \end{multline*}
    We denote by $T_n(f,a,b)$ the approximation of $I$ by trapezoidal rule.
\end{theorem}
\begin{theorem}[Composite Simpson's rule]
    ...
    \end{multline*}
    We denote by $S_n(f,a,b)$ the approximation of $I$ by Simpson's rule.
\end{theorem}
...
\begin{theorem}[Euler–Maclaurin formula]
    ...
    \begin{multline*}
        T_n(f,a,b)=\int_a^bf(t)\dd t+\sum_{k=1}^m\frac{B_{2k}h^{2k}}{(2k)!}\left(f^{(2k-1)}(b)\right.-\\-\left.f^{(2k-1)}(a)\right)+\frac{(b-a)B_{2m+2}h^{2m+1}}{(2m+2)!}f^{(2m+2)}(\xi),
    \end{multline*}
    where $h=\frac{b-a}{n}$, $B_n$ are the Bernoulli numbers and $\xi\in[a,b]$.
\end{theorem}
\begin{theorem}[Romberg method]
    Let $f\in\mathcal{C}^{2m+2}([a,b])$ be a function. Then by Euler–Maclaurin formula, we obtain: $$T_n(f,a,b)=\int_a^bf(t)\dd t+\beta_1 h^2+\beta_2 h^4+\cdots,$$ where $h=\frac{b-a}{n}$. For $n=1,2,4,8,16,\ldots$ we define: $$T_{n,1}=T_{2^n}(f,a,b),\quad T_{n,m+1}=\frac{4^jT_{n+1,m}-T_{n,m}}{4^j-1}.$$ We can observe that $T_{n,m}=\int_a^bf(t)\dd t+O(\left(\frac{h}{2^n}\right)^{2m})$.
\end{theorem}
