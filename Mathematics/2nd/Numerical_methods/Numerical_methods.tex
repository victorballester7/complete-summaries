...
\end{definition}
\begin{definition}
    Let $\mathblack{A}\in\mathcal{M}_n(\RR)$. We define a \textit{subordinated matrix norm} $\|\cdot\|$ as: $$\|\mathblack{A}\|:=\max\{\|\mathblack{A}\mathblack{v}\|_\alpha:\|\mathblack{v}\|_\alpha=1\}=\sup\left\{\frac{\|\mathblack{A}\mathblack{v}\|_\alpha}{\|\mathblack{v}\|_\alpha}:\mathblack{v}\ne 0\right\}.$$ 
\end{definition}
\begin{lemma}
    All subordinated matrix norms are compatible.
\end{lemma}
\begin{prop}
    Let $\mathblack{v}=(v_1,\ldots,v_n)\in\RR^n$. Given the vector norms: 
    \begin{gather*}
    ...
    \end{gather*}
    where $\rho(\mathblack{M})$ denotes the spectral radius of $\mathblack{M}\in\mathcal{M}_n(\RR)$.
\end{prop}
\end{multicols}
\end{document}
