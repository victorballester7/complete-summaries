...
\begin{minipage}{\linewidth} 
    \centering
    \includegraphics[width=\linewidth]{Mathematics/2nd/Numerical_methods/Images/runge.tex}
...
Suppose a function $f$ has a an expansion: $$f(h)=\alpha+a_1h^{p_1}+a_1h^{p_2}+\cdots\quad\text{with }p_1<p_2<\cdots.$$ Given $q>0$, we define $$F_1(h):=F(h),\quad F_{n+1}(h):=\frac{q^{p_n}F_n\left(\frac{h}{q}\right)-F_n(h)}{q^{p_n}-1}.$$ And we can observe that $\alpha=F_n+O(h^{p_n})$.
\end{theorem}
\end{multicols}
\end{document}
