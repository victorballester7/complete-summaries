...
\begin{multicols}{2}[\section{Functions of several variables}]
  ...
  \begin{remark}
    ...
    $$f(x)=\sum_{\abs{\vf\alpha}\geq 0}\frac{\partial^{\vf\alpha}f(a)}{\vf\alpha!}{(x-a)}^{\vf\alpha}$$
    where we the multi-index $\vf\alpha=(\alpha_1,\ldots,\alpha_n)$ is a vector of non-negative integers and $\abs{\vf\alpha}:=\alpha_1+\cdots+\alpha_n$. Moreover:
    \begin{align*}
