...
\begin{multicols}{2}[\section{Harmonic analysis}]
  ...
  \begin{definition}
    Let $\Omega\subseteq \RR^n$ be a set, $T\in\mathcal{D}^*(\Omega)$ and $\alpha$ be a multiindex. We define the distribution $\partial^\alpha T$ as: $$\dotp{\partial^\alpha T}{\varphi}=\dotp{T}{{(-1)}^n\partial^\alpha\varphi}$$
  ...
  \end{lemma}
  \begin{definition}
    Let $T\in \mathcal{S}^*(\RR^d)$. We define the \emph{Fourier transform} $\widehat{T}$ of $T$ as
    $$
      \langle \widehat{T},\varphi\rangle:=\langle T,\widehat{\varphi}\rangle
    $$
    for all $\varphi\in \mathcal{S}(\RR^d)$.
  \end{definition}
\end{multicols}
\end{document}