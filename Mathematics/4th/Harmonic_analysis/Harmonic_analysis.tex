...
\begin{multicols}{2}[\section{Harmonic analysis}]
  ...
  \end{theorem}
  \begin{sproof}
    The three of them are similar, so we only prove the first one. Using \mnameref{HA:minkowski}, we have:
    \begin{align*}
      \norm{\sigma_Rf-f}_p & =\norm{\int_{-\infty}^{\infty}F_R(y)(f(x-y)-f(x))\dd{y}}_p                                                               \\
                           & \leq\int_{-\infty}^{\infty}{\left[\int_{-\infty}^{\infty}{F_R(y)}^p\abs{f(x-y)-f(x)}^p\dd{x}\right]}^{\frac{1}{p}}\dd{y} \\
                           & = \int_{-\infty}^{\infty}F_R(y)\norm{f-T_{y/R}f}_p\dd{y}                                                                 \\
      \begin{split}
        & \leq \int_{\abs{y}< \delta}F_R(y)\norm{f-T_{y/R}f}_p\dd{y}+\\
        &\hspace{3.5cm}+2\norm{f}_p\int_{\abs{y}\geq\delta}F_R(y)\dd{y}
      \end{split}
    \end{align*}
  ...
  \begin{proposition}
    ...
    \begin{enumerate}
      \item $\widehat{(\alpha f+\beta g)}(\vf\xi)=\alpha\widehat{f}(\xi)+\beta \widehat{g}(\vf\xi)$
      ...
      \item If $\lambda\in\RR^*$, then: $$\frac{1}{\lambda^n}\widehat{f\left(\frac{\vf{x}}{\lambda}\right)}(\xi)=\widehat{f}(\lambda\vf\xi)$$
  ...
  \begin{theorem}
    ...
    \begin{enumerate}
      ...
      \item $\displaystyle\int_{-\infty}^{+\infty}f(x)\overline{g}(x)\dd{x}=\int_{-\infty}^{+\infty}\widehat{f}(x)\overline{\widehat{g}}(x)\dd{x}$
    \end{enumerate}
  \end{theorem}
  \begin{proof}
  ...
  \begin{theorem}[Uncertainty principle]
    Let $f\in L^2(\RR)$ such that $xf^2\in L^1(\RR)$ and $f'\in L^2(\RR)$. Then:
  ...
  \begin{proof}
    ...
    Now by the \mnameref{RFA:cauchyschwarz}, we have:
    \begin{align*}
  ...
  \begin{proof}
    ...
    \end{align*}
    The second equality follows from both \mnameref{HA:plancherel,MA:parseval}:
  ...
  \begin{definition}
    ...
    If we denote $\omega_N:=\exp{-\frac{2\pi\ii nk}{N}}$ we can write:
    $$\widehat{f}[k]=\sum_{n=0}^{N-1}f[n]{\omega_N}^n$$
