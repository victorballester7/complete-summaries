...
\begin{multicols}{2}[\section{Harmonic analysis}]
  ...
  \begin{definition}
    Let $(\varphi_n),\varphi\in\mathcal{S}(\RR^d)$. We say that $(\varphi_n)\overset{\mathcal{S}}{\longrightarrow}\varphi$ if $\norm{\varphi_n-\varphi}_{\alpha,\beta}\to 0$ for all $\alpha,\beta\in{(\NN\cup\{0\})}^d$.
  \end{definition}
  \begin{definition}
  ...
  \begin{proof}
    ...
    \end{multline*}
  \end{proof}
  \begin{lemma}\label{HA:xT_equal_0}
    Let $T\in \mathcal{D}*(\RR)$ be such that $x T =0$. Then, $T=C\delta$ for some $C\in \RR$.
  \end{lemma}
  \begin{proof}
    Let $\varphi\in \mathcal{D}(\RR)$ be such that $\varphi(0)=0$. Then, $\phi:=\frac{\varphi}{x}\in \mathcal{D}(\RR)$ and:
    $$
      \langle T,\varphi\rangle=\langle T,x\phi\rangle=\langle xT,\phi\rangle=0
    $$
    Now consider $\omega\in \mathcal{D}(\RR)$ such that $\omega(0)=1$ and $\varphi\in\mathcal{D}(\RR)$ arbitrary. Then, $\phi:=\varphi-\omega\varphi(0)\in \mathcal{D}(\RR)$ and satisfies $\phi(0)=0$. Hence, $\langle T, \phi\rangle=0$ and so:
    $$
      \langle T,\varphi\rangle=\varphi(0)\langle T,\omega\rangle=:C\delta(\varphi)
    $$
  \end{proof}
  ...
  \begin{proof}
    ...
    $$
    By \mcref{HA:xT_equal_0}, we get that $\widehat{\indi{(0,\infty)}}=\frac{1}{2\pi\ii}\pv\left(\frac{1}{x}\right)+C\delta_0$, for some $C\in \RR$. To find the constant change $x\to -x$ in the equation or alternatively apply the distribution to the function $\exp{-\pi x^2}$.
  \end{proof}
  \begin{definition}
