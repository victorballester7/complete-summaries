...
\begin{multicols}{2}[\section{Harmonic analysis}]
  ...
  \begin{definition}
    ...
    $$
      \mathcal{S}(\RR^d):=\{f\in \mathcal{C}^\infty(\RR^n):\norm{f}_{\alpha,\beta}<\infty\ \forall\alpha,\beta\in\NN^d\}
    $$
    where:
    $$
  ...
  \end{definition}
  \begin{lemma}
    Let $f\in \mathcal{S}(\RR^d)$. Then, $x^\alpha f,\partial^\alpha f\in \mathcal{S}(\RR^d)$ for all $\alpha\in\NN^d$.
  \end{lemma}
  \begin{lemma}
    Let $d\in\NN$. Then, $\mathcal{D}(\RR^d)\subset\mathcal{S}(\RR^d)$.
  \end{lemma}
  \begin{definition}
    Let $f,(f_n)\in \mathcal{S}(\RR^d)$. We say that $f_n\overset{S}{\longrightarrow}f$ if $\norm{f_n-f}_{\alpha,\beta}\to 0$ for all $\alpha,\beta\in\NN^d$.
  \end{definition}
  \begin{proposition}
  ...
  \end{lemma}
  \begin{lemma}
    Let $T\in \mathcal{S}^*(\RR^d)$. Then, $\partial^\alpha T\in \mathcal{S}^*(\RR^d)$ for all $\alpha\in\NN^d$.
  \end{lemma}
  \begin{definition}
  ...
  \end{lemma}
  \begin{proof}
    For $m=0$, the equality is true. Now suppose $m\in\NN$ and $b\leq a$. By the binomial theorem we have:
    \begin{align*}
      {(a+b)}^m & =\sum_{k=0}^m \binom{m}{k}a^{m-k}b^k           \\
                & \leq a^m\sum_{k=0}^{m-1} \binom{m}{k}+b^m      \\
                & =a^m(2^m-1)+b^m                                \\
                & =2^{m-1}a^m+2^{m-1}a^m-a^m+b^m                 \\
                & \leq 2^{m-1}(b^m+{(a-b)}^m)+2^{m-1}a^m-a^m+b^m \\
                & \leq  2^{m-1}(a^m+b^m)
    \end{align*}
    where the last inequality follows from:
    $$
      {(2a-2b)}^m\leq 2(a^m-b^m)
    $$
  \end{proof}
  \begin{lemma}
  ...
  \end{lemma}
  \begin{proof}
    Clearly $T*\psi$ is linear. Let $\varphi_n\overset{S}{\longrightarrow}0$. Then, it suffices to see that $\tilde\psi*\varphi_n\overset{S}{\longrightarrow}0$. For all $\alpha,\beta\in \NN^d$ we have:
    \begin{align*}
  ...
  \end{proof}
  \begin{lemma}
    Let $T\in \mathcal{S}^*(\RR^d)$, $\psi\in \mathcal{S}(\RR^d)$ and $\beta\in\NN^d$. Then:
    $$
      \partial^\beta(T*\psi)=\partial^\beta T*\psi=T*\partial^\beta\psi
    $$
  \end{lemma}
  \begin{proof}
    \begin{multline*}
      \langle \partial^\beta(T*\psi),\varphi\rangle={(-1)}^{\abs{\beta}}\langle T*\psi,\partial^\beta\varphi\rangle={(-1)}^{\abs{\beta}}\langle T,\psi*\partial^\beta\varphi\rangle\\={(-1)}^{\abs{\beta}}\langle T,\partial^\beta(\psi*\varphi)\rangle
      =\langle \partial^\beta T,\psi*\varphi\rangle   =\langle \partial^\beta T*\psi,\varphi\rangle
    \end{multline*}
  ...
  \begin{proposition}
    Let $T\in \mathcal{S}^*(\RR^d)$, $\psi \in \mathcal{S}(\RR^d)$ and $\beta\in\NN^d$. Then:
    \begin{enumerate}
      \item $\partial^\beta\widehat{T}=\F({{(-2\pi\ii x)}^{\beta}\partial^\beta T})$
      \item $\widehat{\partial^\beta T}={(2\pi\ii \xi)}^{\beta}\widehat{T}$
