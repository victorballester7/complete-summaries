...
\begin{multicols}{2}[\section{Harmonic analysis}]
  ...
  \begin{proposition}
    ...
    \begin{enumerate}
      ...
      \item For all $\delta>0$, we have:
            ...
            \end{multline*}
    \end{enumerate}
    That is, $F_R$, $P_t$ and $W_t$ are \emph{approximations of the unity}.
  \end{proposition}
  \begin{sproof}
  ...
  \end{definition}
  \begin{proposition}
    Let $\Omega\subseteq \RR^n$ be a set and $\phi\in L^1(\Omega)$ such that $\int_\Omega\phi=1$. Let $\phi_\varepsilon:=\frac{1}{\varepsilon^n}\phi(\frac{\vf{x}}{\varepsilon})$. Then, $(\phi_\varepsilon)$ is an approximation of unity, $\phi_\varepsilon \in L_\mathrm{loc}^1(\Omega)$ $\forall \varepsilon>0$ and $\phi_\varepsilon\overset{\varepsilon\to 0}{\longrightarrow} \delta_{\vf{0}}$ in $\mathcal{D}^*(\Omega)$.
  \end{proposition}
  \begin{sproof}
  ...
  \begin{definition}
    Let $T\in\mathcal{D}^*(\RR^n)$. We define the \emph{support} of $T$, $\supp T$, as the intersection of all compact sets such that if $\varphi\in\mathcal{D}(\RR^n)$ has support in $\RR^n\setminus K$, then $\langle T, \varphi\rangle=0$.
  \end{definition}
  ...
  \end{definition}
  \begin{definition}
    Let $T\in\mathcal{D}^*(\RR^n)$. We can extent the definition of $T$ to $\mathcal{E}(\RR^n)$ in the following way. Let $\varphi\in\mathcal{E}(\RR^n)$ and take $\rho\in\mathcal{D}(\RR^n)$ such that $\rho=1$ on $\supp\varphi$. Then, we define:
    $$
      \langle T,\varphi\rangle := \langle T,\rho\varphi\rangle
    $$
  \end{definition}
  \begin{remark}
    Note that in view of \mcref{HA:supp_well_def}, this definition is well defined because if $\rho,\omega\in\mathcal{D}(\RR^n)$ are two different test functions such that $\rho,\omega=1$ on $\supp\varphi$, then $\varphi(\rho- \omega)=0$ on $\supp\varphi$ and therefore $\langle T,\varphi(\rho- \omega)\rangle=0$.
  \end{remark}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      \end{split}
    \end{align*}
    where in the second inequality we have used \mcref{HA:lemma_aMbM}. Note that the this latter terms tend to zero as $n\to\infty$ because of the properties of the Schwartz space.
  \end{proof}
  \begin{lemma}
  ...
  \end{definition}
  \begin{theorem}
    Let $L(\partial)$ be a differential operator. Then, $E*f$ is a solution of the Cauchy problem \mcref{HA:cauchy}, where $E$ is a fundamental solution of it.
  \end{theorem}
  ...
  \begin{proof}
    ...
    \begin{align*}
      \langle \laplacian\log\norm{\vf{x}},\varphi\rangle & = \lim_{n\to\infty}\langle \laplacian E_n,\varphi\rangle                                        \\
                                                         ...
                                                         & =2\pi\varphi(0)
    \end{align*}
    where in the first and forth equality we have used the \mnameref{RFA:dominated} and at the end we have calculated the integral using polar coordinates.
  \end{proof}
  \begin{corollary}
