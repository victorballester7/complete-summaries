...
\begin{multicols}{2}[\section{Partial differential equations}]
  ...
  \begin{proposition}\label{PDE:preexistence}
    Let $\Omega\subseteq \RR^n$ be a bounded subset and $\tilde{g}\in H(\Omega)$. Then, $\exists! \tilde{u}\in {H_0(\Omega)}^\perp=H\ominus H_0(\Omega)$ such that $$\tilde{u}=\argmin_{w\in H_0(\Omega)}\left\{\idotsint_\Omega\frac{1}{2}\norm{\grad w}^2:\tilde{g}-w\in H_0(\Omega)\right\}$$
  \end{proposition}
  \begin{sproof}
