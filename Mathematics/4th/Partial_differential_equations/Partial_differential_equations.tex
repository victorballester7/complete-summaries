...
\begin{multicols}{2}[\section{Partial differential equations}]
  ...
  \end{proposition}
  \begin{sproof}
    Let $a(t)\leq x_\mathrm{s}\leq b(t)$ and $t_2\geq t_1$. Then, using \cref{PDE_trafficintegral2} we have:
    \begin{multline*}
      \int_{a(t_2)}^{b(t_2)}\rho(x,t_2)\dd{x}-\int_{a(t_1)}^{b(t_1)}\rho(x,t_1)\dd{x}=\\=\int_{t_1}^{t_2}[q(a(t),t)-q(b(t),t)]\dd{t}=\\=\int_{t_1}^{t_2}[\rho(a(t),t)(u-a') -\rho(b(t),t)(u-b')]\dd{t}
    \end{multline*}
  ...
  \end{proposition}
  \begin{sproof}
    We already saw the expressions of $\phi$ and $\psi$ for $x\geq 0$ in \nameref{PDE_propdAlembert}. For $x<0$, note that: $$\beta(t)=u_t(0,t)=c\phi'(ct)-c\psi'(-ct)$$
  \end{sproof}
  ...
  \begin{definition}
    Let $\Omega$ be a set. We define the space $\mathcal{C}_0^\infty(\Omega)=:\mathcal{D}(\Omega)$ as the set of all functions in $\mathcal{C}^\infty(\Omega)$ that have compact support. We define the space $L_{\mathrm{loc}}^1(\Omega)$ as the set of all locally integrable functions on $\Omega$.
  \end{definition}
  \begin{definition}[Distribution]
    Let $\Omega$ be a set. A \emph{distribution} on $\Omega$ is a continuous linear functional on $\mathcal{D}(\Omega)$. The vector space of all distributions on $\Omega$ is denoted by $\mathcal{D}^*(\Omega)$.
  \end{definition}
  \begin{proposition}
    Let $\Omega$ be a set and $f\in L_{\mathrm{loc}}^1(\Omega)$. Then, the map
    $$
      \function{\Lambda_f}{\mathcal{D}(\Omega)}{\RR}{\varphi}{\displaystyle\int_\Omega f(x)\varphi(x)\dd{x}}
    $$
    is a distribution. Hence, $\Lambda_f(\varphi)$ is usually denoted by $\dotp{f}{\varphi}$. Sometimes we will do an abuse of notation denoting $\Lambda_f$ as $f$.
  \end{proposition}
  \begin{proof}
    $\Lambda_f$ is clearly linear. Moreover: $$\abs{\Lambda_f(\varphi)}\leq\int_\Omega\abs{f(x)\varphi(x)}\leq \norm{f}_1\norm{\varphi}_\infty$$
    Hence, $\Lambda_f$ is bounded and therefore continuous.
  \end{proof}
  \begin{proposition}[Dirac's $\delta$ distribution]
    Let $\Omega$ be a set and $x_0\in\Omega$. Then, the map
    $$
      \function{\delta_{x_0}}{\mathcal{D}(\Omega)}{\RR}{\varphi}{\varphi(x_0)}
    $$
    is a distribution. Note that, we can write $$\Lambda_{\delta_{x_0}}=\dotp{\delta_{x_0}}{\varphi}=\int_\Omega\delta_{x_0}(x)\varphi(x)\dd{x}=\varphi(x_0)=\delta_{x_0}(\varphi)$$ We will denote $\delta_0$ simply by $\delta$.
  \end{proposition}
  \begin{proof}
    Clearly $\delta_{x_0}$ is linear and bounded because $\abs{\delta_{x_0}(\varphi)}=\abs{\varphi(x_0)}\leq \norm{\varphi}_\infty$.
  \end{proof}
  \begin{proposition}
    Let $\Omega$ be a set and $x_0\in\Omega$. Then: $$\int_\Omega \delta_{x_0}(x)\dd{x}=1$$
  \end{proposition}
  \begin{proof}
    $$\int_\Omega \delta_{x_0}(x)\dd{x}=\dotp{\delta_{x_0}}{1}=1(x_0)=1$$
  \end{proof}
  \begin{definition}
    Let $\Omega$ be a set and $n\in\NN$. We define the \emph{differentiation operator} $D^n:\mathcal{D}^*(\Omega)\rightarrow\mathcal{D}^*(\Omega)$ by: $$\dotp{D^n\Lambda}{\varphi}=\dotp{\Lambda}{{(-1)}^nD^n\varphi}$$
  ...
  \begin{proof}
    ...
    \begin{multline*}
      ...
      =-\int_{0}^\infty\varphi'(x)\dd{x}= \varphi(x_0)=\delta_{x_0}(\varphi)
    \end{multline*}
  ...
  \end{corollary}
  \begin{sproof}
    Clearly the heat equation holds by construction. The proof of $\displaystyle\lim_{t\to 0}\abs{H(t)*f - f}=0$ follows in the same way as the one in \cref{PDE_heatkernelprop}.
  \end{sproof}
  \begin{proposition}
