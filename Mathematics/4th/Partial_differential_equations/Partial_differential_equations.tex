...
\begin{multicols}{2}[\section{Partial differential equations}]
  ...
  \end{proposition}
  \begin{sproof}
    For the first one apply \nameref{FSV_divergencethm} with the vector field $\vf{X}_1 = kf\grad{g}$ and for the second one, use the previous formula and the symmetry of $f$ and $g$.
  \end{sproof}
  ...
  \end{theorem}
  \begin{sproof}
    Apply \nameref{FSV_green} to the vector field $\vf{X}= (\rho u_t,k u_x)$.
  \end{sproof}
  \begin{proposition}
  ...
  \begin{definition}[Explicit scheme in finite differences]
    ...
    \begin{equation}\label{PDE_operatorpde}
      ...
      \right.
    \end{equation}
    We would like to extend the notion of \cref{DE_matrixexp}. Thus for $n\gg t$ we can rewrite the previous equation as:
