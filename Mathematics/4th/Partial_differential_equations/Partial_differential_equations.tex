...
\begin{multicols}{2}[\section{Partial differential equations}]
  ...
  \begin{proof}
    ...
    If there were a point $\vf{x}_0\in \Omega$ such that (without loss of generality) $f(\vf{x}_0)>0$, the continuity would imply the existence of an open set $U$ containing $\vf{x}_0$ and a $\varepsilon >0$ such that $f(\vf{x})>\varepsilon$ $\forall \vf{x}\in U$. Now take $\varphi\in\mathcal{C}_0^\infty(\Omega)$ such that $\varphi\geq 0$, $\supp\varphi\subset U$ and $\varphi>0$ in some open set $V\subseteq U$. And we would have: $$0=\dotsint_\Omega f(\vf{x})\varphi(\vf{x})\dd{\vf{x}}=\dotsint_U f(\vf{x})\varphi(\vf{x})\dd{\vf{x}}>0$$
  \end{proof}
  \begin{proposition}
  ...
  \begin{proof}
    ...
    with $\varepsilon>0$ such that $T+\varepsilon<\frac{1}{4a}$ and $\delta>0$. It can be checked that $v_t- \laplacian v= 0$. Now fix $r>0$ and consider the set $U_T=B(\vf{y}, r)\times(0,T]$. By the \mnameref{PDE:max}, we have that: $$\max\{v(\vf{x},t):(\vf{x},t)\in\overline{U_T}\}=\max\{v(\vf{x},t):(\vf{x},t)\in\Gamma_T\}=:M$$
