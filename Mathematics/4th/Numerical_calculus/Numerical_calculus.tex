...
\begin{multicols}{2}[\section{Numerical calculus}]
  ...
  \begin{definition}[Multiple shooting method]
    Suppose we want to solve the problem of \mcref{NC:bvp} and consider the partition of the time-interval of integration $t_0<t_1<\cdots<t_N$. The method starts by guessing the solution $\vf{x}_k$ of the BVP at $t_k$, $k=0,\ldots,N$. Now let, $\vf{x}(t;\vf{x}_k,t_k)$ be the solution to the ivp:
    \begin{equation*}
      \begin{cases}
        \vf{x}' =f(t, \vf{x}) \\
        \vf{x}(t_k)=\vf{x}_k
  ...
  \begin{definition}[Singular value]
    Let $m\geq n$ and $\vf{A}\in\mathcal{M}_{m\times n}(\RR)$. The \emph{singular values} of $\vf{A}$ are the square roots of the eigenvalues of $\transpose{\vf{A}}\vf{A}$.
  \end{definition}
  ...
  \end{lemma}
  \begin{proof}
    Let $\vf{v}$ be an unitary eigenvector of $\vf{Q}$ associated to $\lambda$. Then:
  ...
  \end{lemma}
  \begin{proof}
    Note that $\vf{Q} \transpose{\vf{Q}}=\vf{I}_n$. Taking determinants, we obtain the first equality. The second equality, follows from \mcref{NC:eigenvalues_orto,NC:svd-cor}.
  \end{proof}
  \begin{definition}[QR descompostion]
    Let $\vf{A}\in\mathcal{M}_{m\times n}(\RR)$ be a matrix. A \emph{QR decomposition} of $\vf{A}$ is an expression $\vf{A}=\vf{Q}\vf{R}$, where $\vf{Q}\in\mathcal{O}_n(\RR),\vf{R}\in\mathcal{M}_n(\RR)$ are such that $\vf{Q}$ is orthogonal and $\vf{R}$ is upper triangular.
  \end{definition}
  \begin{lemma}
