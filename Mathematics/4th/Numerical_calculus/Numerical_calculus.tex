...
\begin{multicols}{2}[\section{Numerical calculus}]
  ...
  \begin{definition}
    ...
    $$
    for some function $g:\RR\rightarrow\RR$. This function is called \emph{stability function}. Given $h$ and $\lambda$, the method is said to be \emph{stable} if $\abs{g(h\lambda)}\leq 1$ and \emph{absolutely stable} if $\abs{g(h\lambda)}< 1$ (that is, $h\lambda\in\mathcal{A}$). If $\abs{g(h\lambda)}> 1$, the method is said to be \emph{unstable}.
  \end{definition}
  \begin{definition}
  ...
  \begin{proof}
    ...
    \begin{multline*}
      \vf{e}_{n+1}=\vf{y}_{n+1}-\vf{\tilde{y}}_{n+1}=\vf{y}_{n+1}-g(h\lambda)\vf{\tilde{y}}_n=\\
      =\vf{y}_{n+1}-g(h\lambda)(\vf{y}_n-\vf{e}_n)=h\vf{\tau}_n(h)+g(h\lambda)\vf{e}_n
    \end{multline*}
    Iterating the proces and taking norms we have:
    $$
      \norm{\vf{e}_n}\leq\sum_{j=0}^n{\abs{g(h\lambda)}}^jh\norm{\vf{\tau}_n(h)}\leq nh\tau(h)\overset{h\to 0}{\longrightarrow}0
    $$
  ...
  \begin{definition}[Adams method]
    ...
    The coefficients $\beta_j$ are found by integrating the Lagrange interpolating polynomial between $t_{n+k-1}$ and $t_{n+k}$ constructed from the nodes $(t_{n+j},\vf{\tilde{f}}_{n+j})$ for $j=0,\ldots,k$, for the Adams-Bashforth method, and for $j=0,\ldots,k-1$ for the Adams-Moulton method.
  \end{definition}
  \begin{definition}
