...
\begin{multicols}{2}[\section{Stochastic processes}]
  ...
  \begin{definition}
    ...
    \end{equation}
    The set $\mathcal{D}_X$ is defined as a all the points for which the series of \mcref{SP:probgenfunc} converges absolutely.
  \end{definition}
  \begin{lemma}
  ...
  \begin{definition}
    ...
    then we say that the Markov chain is a \emph{time-homogeneous Markov chain}.
  \end{definition}
  ...
  \end{proof}
  \begin{definition}
    Let $(X_n)$ be a time-homogeneous Markov chain. A subset $C\subseteq I$ is called \emph{irreducible class} if for any $i,j\in C$ we have $i\leftrightarrow j$. That is, if $C$ is a equivalence class of $\leftrightarrow$. If all the states are in the same equivalence class, then $C$ is called an \emph{irreducible chain}.
  \end{definition}
  \begin{definition}
  ...
  \end{definition}
  \begin{proposition}
    Let $(X_n)$ be a time-homogeneous Markov chain. Then: $$i\leftrightarrow j\implies d(i)=d(j)$$
  \end{proposition}
  \begin{proof}
    ...
    So $d(i)\mid r+n+s$. Thus, $d(i)\mid n$ and so $d(j)\geq d(i)$ because $\{n\in\NN:p_{jj}^{(n)}>0\}=d(i)\ZZ$. Repeating the argument exchanging $i$ and $j$ we get $d(j)= d(i)$.
  \end{proof}
  \begin{definition}
    Let $(X_n)$ be a time-homogeneous Markov chain. If the chain of states is irreducible, we will denote the common period by $d$. If $d=1$ we say that the chain is \emph{aperiodic}.
  \end{definition}
  \begin{proposition}
    Let $(X_n)$ be a time-homogeneous Markov chain. Suppose we have an irreducible chain of period $d>1$. Then, there exist subsets $C_0,\ldots,C_{d-1}\subseteq I$ such that $I=C_0\sqcup\cdots\sqcup C_{d-1}$ and such that if $i\in C_\alpha$, then:
    $$p_{ij}>0\implies j\in C_{{[\alpha+1]}_d}$$
    for all $j\in I$. Here ${[\alpha+1]}_d$ denotes $\alpha+1\mod{d}$.
  \end{proposition}
  \begin{proof}
  ...
  \end{proposition}
  \begin{proof}
    By \mcref{SP:lema2Markov} it suffices to prove the statement for $B=\{i_0\}\times\cdots\times\{i_n\}$. Moreover since $A$ is countable we can suppose $A=\{j_1\}\times\cdots\times\{j_k\}$. We will prove it by induction on $k$ the homogeneous equality (the other one is even easier). The case $k=1$ is by definition. Now suppose $k\geq 2$. Then, denoting $C:=\{X_0=i_0,\ldots,X_{n-1}=i_{n-1},X_n=i\}$ we have:
    \begin{multline*}
  ...
  \end{proof}
  \begin{definition}
    Let $(\Omega,\mathcal{F},(\mathcal{F}_n)_{n\geq 0},\Prob)$ be a filtration space and $\tau$ a random variable on it. We say that $\tau$ is a \emph{stopping time} if $\forall n\geq 0$ we have:
  ...
  \end{proof}
  \begin{proposition}
    Let $(X_n)$ be a time-homogeneous Markov chain, $(\Omega,\mathcal{F},(\mathcal{F}_n)_{n\geq 0},\Prob)$ be a filtration space defined with $(X_n)$, $i\in I$ and $\tau_i$ be the random variable, whose outcomes are in $\NN\cup\{0,\infty\}$, defined by:
  ...
  \begin{theorem}[Strong Markov property]\label{SP:MarkovStrong}
    ...
    \begin{multline*}
      \Prob(X_{\tau+n+1}=j\mid X_{\tau}=i_0,\ldots,X_{\tau+n-1}=i_{n-1},X_{\tau +n}=i,\\\tau<\infty)=\Prob(X_{\tau+n+1}=j\mid X_{\tau+n}=i,\tau<\infty)=\\=\Prob(X_{n+1}=j\mid X_n=i)
    \end{multline*}
  ...
  \begin{definition}
    ...
    $$
      N_i:=\abs{\{n\in\NN:X_n=1\}}=\sum_{n=1}^{\infty}\indi{\{X_n=i\}}
    $$
  \end{definition}
  \begin{remark}
  ...
  \end{remark}
  \begin{definition}
    Let $(X_n)$ be a time-homogeneous Markov chain and $i\in I$.
  \end{definition}
  ...
  \begin{proof}
    ...
    $$
    with the convention that $\tau_i^1=\tau_1$. An easy check shows that $\tau_i^k$ is a stopping time $\forall k\in\NN$ and $\{\tau_i^k<\infty\}\subseteq \{\tau_i^{k-1}<\infty\}$. Thus:
    \begin{multline*}
