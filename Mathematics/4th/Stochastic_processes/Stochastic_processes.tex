...
\begin{multicols}{2}[\section{Stochastic processes}]
  ...
  \begin{definition}
    ...
    then we say that the Markov chain is a \emph{time-homogeneous Markov chain}. The elements of $I$ are called \emph{states} of the Markov chain.
  \end{definition}
  ...
  \begin{definition}
    ...
    We define the vector $\vf{\pi}^{(n)}=(\pi_i^{(n)})_{i\in I}$ as the \emph{$n$-step distribution} of the Markov chain.
  \end{definition}
  \begin{definition}
  ...
  \begin{definition}
    Let $(X_n)$ be a time-homogeneous Markov chain. We say that a state $j\in I$ is \emph{reachable} from $i\in I$ if $\exists n\in I$ such that $p_{ij}^{(n)}>0$. In this case we will write $i\to j$.
  \end{definition}
  \begin{definition}
  ...
  \begin{proof}
    ...
    So $d(i)\mid r+n+s$. Thus, $d(i)\mid n$ and so $d(j)\geq d(i)$ because $\{n\in\NN:p_{jj}^{(n)}>0\}=d(i)(\NN\cup\{0\})$. Repeating the argument exchanging $i$ and $j$ we get $d(j)= d(i)$.
  \end{proof}
  \begin{definition}
