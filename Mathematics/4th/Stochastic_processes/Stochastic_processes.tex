...
\begin{multicols}{2}[\section{Stochastic processes}]
  ...
  \begin{proof}
    \begin{enumerate}
      ...
      \item If $i\leftrightarrow j$, then $\exists r,s\geq 1$ such that $p_{ij}^{(r)}, p_{ji}^{(s)}>0$. So by \mcref{SP:corolariChapKolmo} we have that $\forall n\geq 0$:
            ...
            $$
            And so, $\sum p_{ii}^{(n)}=\infty\implies \sum p_{jj}^{(n)} = \infty$ by \mcref{thmRecA}.
      ...
      \item It follows from \mcref{thmRecA} and \mcref{SP:lemaPrerec}.
    \end{enumerate}
  \end{proof}
  \begin{definition}
  ...
  \begin{proof}
    ...
    $$
    where the last inequality follows from \mcref{thmRecD}.
  \end{proof}
  ...
  \begin{proof}
    ...
    FALTACOSA
    Observe that $\frac{N_i^n}{n}\leq 1$. Thus, by the \mcref{P:dominated} we have that:
    $$
