...
\begin{multicols}{2}[\section{Numerical integration of partial differential equations}]
  ...
  \begin{definition}[Mesh]
    A \emph{mesh} is a geometric decomposition of a domain $\Omega$ into a finite collection of cells ${\{K_i\}}_{i=1}^N$ such that:
    \begin{enumerate}
  ...
  \end{definition}
  \begin{definition}
    Given the location $x_i$ of $M$ \emph{nodes} in $\Int\Omega$, we define the \emph{nodal basis} $(\phi_1,\ldots,\phi_M)$ as:
    $$\phi_i(x_j)=\delta_{ij}$$
  \end{definition}
