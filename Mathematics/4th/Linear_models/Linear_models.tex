...
\begin{multicols}{2}[\section{Linear models}]
  ...
  \begin{definition}
    We say that $\vf{x}\sim N_n(\vf\mu,\vf\Sigma)$, where $\vf\mu\in\RR^n$ and $\vf\Sigma\in\mathcal{M}_n(\RR)$ is symmetric and positive definite, if its moment generating function\footnote{Remember definition \mcref{P:moment-generating}.} is: $$\psi_{\vf{x}}(\vf{u})=\exp{\transpose{\vf{\mu}}\vf{u}}\exp{\frac{1}{2}\transpose{\vf{u}}\vf\Sigma\vf{u}}\qquad\forall\vf{u}\in\RR^n$$
  \end{definition}
  \begin{proposition}
  ...
  \begin{proposition}[Least-squares method]\label{LM:least-squares}
  ...
  \begin{axiom}[Information]\label{LM:info}
    ...
    \end{itemize}
  \end{axiom}
  \begin{theorem}[Cauchy's functional equation]\label{LM:cauchy_functional}
  ...
  \end{lemma}
  \begin{proof}
    By the first axiom of \mnameref{LM:info} we have that $I(A)=f(\Prob(A))$ and by the third one that if $A$, $B$ are independent we have that: $$f(\Prob(A)\Prob(B))=f(\Prob(A\cap B))=f(\Prob(A))+f(\Prob(B))$$
    That is, $f(xy) = f(x)+f(y)$ $\forall x,y\in[0,1]$. Now consider $g(x)=f(\exp{x})$. Then:
    $$g(x+y)=f(\exp{x}\exp{y})=f(\exp{x})+f(\exp{y})=g(x)+g(y)$$
    So by \mnameref{LM:cauchy_functional}, we have that $g(x)=cx$ and so $f(x)=c\log x$ for some $c\in\RR$. The second axiom of \mnameref{LM:info} implies $c<0$.
  \end{proof}
  ...
  \begin{definition}[Akaike information criterion]
    Consider the model of \mcref{LM:multiple}. The \emph{Akaike information criterion} (\emph{AIC}) is an estimator of prediction error that is defined as: $$\mathrm{AIC}=2k-2\log(\Phi_n(y;\vf{\hat\beta},\hat\sigma^2))$$
