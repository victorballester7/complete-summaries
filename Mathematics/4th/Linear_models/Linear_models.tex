...
\begin{multicols}{2}[\section{Linear models}]
  ...
  \end{theorem}
  \begin{proposition}
    Consider the model $\vf{y}\sim N_n(\vf{X\beta},\sigma^2\vf{I}_n)$ of \cref{LM_generalized} and suppose we want to test the hypothesis $\mathcal{H}_0:\vf\beta_1=\vf{0}$ versus $\mathcal{H}_1=\vf\beta_1\ne\vf{0}$. The statistic that we take is: $$F=\frac{\SSR/ k}{\SSE/(n-k-1)}\sim F_{k,n-k-1}$$
  \end{proposition}
  \begin{proposition}
    Consider the model $\vf{y}\sim N_n(\vf{X\beta},\sigma^2\vf{I}_n)$ of \cref{LM_generalized}, $\vf{R}\in\mathcal{M}_{r\times {k+1}}(\RR)$ where $r\leq k+1$ and suppose we want to test the hypothesis $\mathcal{H}_0:\vf{R}\vf\beta_1=\vf{0}$ versus $\mathcal{H}_1=\vf{R}\vf\beta_1\ne\vf{0}$.
    Without loss of generality suppose we can eliminate the first $\beta_1,\ldots,\beta_r$ under $\mathcal{H}_0$ and so rearranging the model equation we obtain:
    $$\vf{y}_\ell=\vf{X}_\ell\vf{\beta}_\ell+\vf\varepsilon$$
    where $\vf{\beta}_\ell=(\beta_{r+1},\ldots,\beta_k)$. The statistic that we take is: $$F=\frac{(\SSE_\ell-\SSE)/ r}{\SSE/(n-k-1)}\sim F_{r,n-k-1}$$
    where the subindex in $\SSE_\ell$ indicates that this is the $\SSE$ for the model under $\mathcal{H}_0$.
  \end{proposition}
\end{multicols}
\end{document}