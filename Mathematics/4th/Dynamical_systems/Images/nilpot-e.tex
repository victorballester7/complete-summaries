\documentclass{standalone}
\usepackage{../../../../preamble_tikz}

\begin{document}
\begin{tikzpicture}[decorated curves/.style={
        postaction={
            decorate,
            decoration={
                markings,
                mark=at position 0.333 with {\arrow[black,scale=1.5]{stealth}},
                mark=at position 0.666 with {\arrow[black,scale=1.5]{stealth}}}}
      },decorated curves rev/.style={
        postaction={
            decorate,
            decoration={
                markings,
                mark=at position 0.333 with {\arrow[black,scale=1.5]{stealth[reversed]}},
                mark=at position 0.666 with {\arrow[black,scale=1.5]{stealth[reversed]}}}}
      },decorated separatrices rev/.style={
        postaction={
            decorate,
            decoration={
                markings,
                mark=at position 0.5 with {\arrow[scale=1.5]{stealth[reversed]}},
              }}
      },decorated separatrices/.style={
        postaction={
            decorate,
            decoration={
                markings,
                mark=at position 0.5 with {\arrow[scale=1.5]{stealth}},
              }}
      }]
  \begin{polaraxis}[
      % xmin=-4, xmax=4,
      % ymin=-4, ymax=4,
      xtick=\empty, ytick=\empty,
      ylabel={$y$},
      xlabel={$x$},
      %axis on top,
      hide axis,
    ]

    \def\alph{0.5}
    \addplot[thick,samples=1000,domain=0:1000,black,decorated separatrices] {\alph};
    \addplot[thick,samples=1000,domain=0:1000,black,decorated separatrices] {\alph+1};
    \addplot[thick,samples=1000,domain=0:1000,black,decorated separatrices] {\alph+2};

    \addplot[thin,mark=*,only marks, mark size=0.1136cm,black] coordinates {
        (0,0)
      };
  \end{polaraxis}
\end{tikzpicture}
\end{document}