...
\begin{multicols}{2}[\section{Dynamical systems}]
  ...
  \end{proposition}
  \begin{proof}
    Let $\varepsilon>0$ and $s,t\in\Sigma$ with $d_2(s,t)<\delta$. Then: $$d_2(\sigma(s),\sigma(t))=\abs{s_0-t_0} + 2d_2(\sigma(s),\sigma(t))<\varepsilon$$ if $\delta<\frac{\varepsilon - \abs{s_0-t_0}}{2}$.
  \end{proof}
  \begin{proposition}
  ...
  \begin{sproof}
    \begin{enumerate}
      \item Just note that there are $2^n$ different sequences in $\Sigma$ of the form:
      ...
      \item Consider the sequence of ``all sequences":
            ...
            \end{multline*}
      \item Let $\varepsilon>0$, take $k\in\NN$ such that $\sum_{n=k}^{\infty}\frac{1}{2^n}<\varepsilon$ and let $s=(s_n)\in\Sigma$. The element $$t_s:=(s_0,\ldots,s_{k-1},s_0,\ldots,s_{k-1},s_0,\ldots,s_{k-1},\ldots)$$
