...
\begin{multicols}{2}[\section{Dynamical systems}]
  ...
  \end{theorem}
  \begin{proposition}
    Consider a system with a heteroclinic cycle $\Gamma$ and such that each of the $n$ equilibrium points that are in $\Gamma$ are linear saddles with eigenvalues $\lambda_i$ and $\mu_i$. Then:
    \begin{enumerate}
      \item If $\displaystyle\prod_{i=1}^n\frac{\mu_i}{\lambda_i}<1$, then $\Gamma$ is unstable.
      \item If $\displaystyle\prod_{i=1}^n\frac{\mu_i}{\lambda_i}>1$, then $\Gamma$ is stable.
    \end{enumerate}
  \end{proposition}
