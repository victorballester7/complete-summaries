...
\begin{multicols}{2}[\section{Real and functional analysis}]
  ...
  \begin{definition}
    Let $E$, $F$ be normed spaces. An \emph{operator} $T$ is a linear function $T:E\rightarrow F$.
  \end{definition}
  \begin{definition}
    Let $E$, $F$ be normed spaces and $T:E\rightarrow F$ be an operator. We say that $T$ is a \emph{compact operator} if $T(B_E)$ is relatively compact on $F$, where: $$B_E:=\{x\in E:\norm{x}_E\leq 1\}$$
  \end{definition}
  \begin{definition}
    Let $E$, $F$ be normed spaces and $T:E\rightarrow F$ be an operator. We define the \emph{norm} of $T$ as: $$\norm{T}:=\sup\{\norm{T(x)}_F:\norm{x}_E\leq 1\}$$
  \end{definition}
  \begin{lemma}
    Let $E$, $F$ be normed spaces and $T:E\rightarrow F$ be an operator. Then:
    $$\norm{T}=\sup\{\norm{T(x)}_F:\norm{x}_E< 1\}=\sup\left\{\frac{\norm{T(x)}_F}{\norm{x}_E}:x\ne 0\right\}$$
  \end{lemma}
  \begin{theorem}
    Let $E$, $F$ be normed spaces and $T:E\rightarrow F$ be an operator. The following are equivalent:
    \begin{enumerate}
