...
\begin{multicols}{2}[\section{Real and functional analysis}]
  ...
  \end{definition}
  \begin{definition}
    Let $E$, $F$ be normed vector spaces and $T:E\rightarrow F$ be an operator. We say that $T$ is a \emph{compact operator} if $T(B_E)$ is relatively compact on $F$, where: $$B_E:=\{x\in E:\norm{x}_E\leq 1\}$$
  \end{definition}
  \begin{definition}
  ...
  \end{theorem}
  \begin{corollary}
    Let $(H,\dotp{\cdot}{\cdot})$ be a Hilbert space and $F\subseteq H$ be a closed subspace. The funcion $P_F:H\rightarrow H$, called \emph{orthogonal projection on $F$}, satisfy the following properties:
    \begin{enumerate}
      \item ${P_F}^2 = P_F$
      \item $\dotp{P_Fx_1}{x_2}=\dotp{x_1}{P_Fx_2}$ $\forall x_1, x_2\in H$
      \item $P_F$ is linear and $\norm{P_Fx}\leq \norm{x}$ and the equality holds if and only if $x\in F$.
      \item If $F\ne\{0\}$, $\norm{P_F}=1$
