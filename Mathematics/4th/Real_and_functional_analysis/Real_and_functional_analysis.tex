...
\begin{multicols}{2}[\section{Real and functional analysis}]
  ...
  \begin{definition}[Measure]
    ...
    \begin{itemize}
      ...
      \item \emph{$\sigma$-additivity}: If $(A_n)\in\Sigma$ are pairwise disjoint, then: $$\mu\left(\bigsqcup_{n=1}^\infty A_n\right)=\sum_{n=1}^\infty \mu(A_n)$$
    \end{itemize}
  \end{definition}
  \begin{definition}
  ...
  \begin{proposition}
    ...
    \begin{enumerate}
      ...
      \item If $A_n\searrow A$ and $\mu(A_1)<\infty$, then $\displaystyle\mu(A)=\lim_{n\to\infty} \mu(A_n)$.
    \end{enumerate}
  \end{proposition}
  \begin{sproof}
    \begin{enumerate}
  ...
  \begin{sproof}
    ...
    \begin{equation*}
      E(k,m)=E(h,m+1)\sqcup E(h+1,m+1)
    \end{equation*}
    provided that $h=2k-1<m2^{m+1}$, and:
    \begin{equation*}
      F(m)=\left(\bigsqcup_{h=m2^{m+1}+1}^{(m+1)2^{m+1}}E(h,m+1)\right)\sqcup F
    \end{equation*}
