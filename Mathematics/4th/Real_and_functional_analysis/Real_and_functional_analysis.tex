...
\begin{multicols}{2}[\section{Real and functional analysis}]
  ...
  \end{lemma}
  \begin{proof}
    By hypothesis $\exists g\in A$ such that $g(x)\ne g(y)$. Then consider:
    $$f=\alpha\frac{g-g(y)}{g(x)-g(y)}+\beta\frac{g-g(x)}{g(y)-g(x)}$$
    Note that $f\in A$ because $A$ vanishes nowhere and so the constants functions are in $A$.
  \end{proof}
  \begin{definition}
  ...
  \begin{theorem}
    ...
    \begin{enumerate}
      ...
      \item $\exists A\subseteq E$ such that it is a countable intersection of dense open subsets such that: $$\sup\{\norm{T_ix}_F:i\in I\}=\infty\quad\forall x\in A$$
    \end{enumerate}
  \end{theorem}
