...
\begin{multicols}{2}[\section{Mechanics and special relativity}]
  ...
  \begin{proposition}[Projectile motion]
    The equations of a projectile motion like the one in \mcref{MSR_projectile} are:
    \begin{align*}
  ...
  \begin{proposition}[Elastic force]
    Consider an object attached to a string of natural length $x_0$ as shown in the \mcref{MSR_elastic_force}.
    \begin{center}
      \begin{minipage}{\linewidth}
        \centering
  ...
  \begin{corollary}
    ...
    \begin{itemize}
      \item The source moves away, that is making $\phi=\pi$ in \mcref{MSR_dopp1} (\emph{Redshift}):
            $$\nu_R=\nu_S\sqrt{\frac{1-\beta}{1+\beta}}$$
      \item The source gets close, that is making $\phi=0$ in \mcref{MSR_dopp1} (\emph{Blueshift}):
            $$\nu_R=\nu_S\sqrt{\frac{1+\beta}{1-\beta}}$$
      \item The source moves transversely, that is making $\phi=\pi/2$ in \mcref{MSR_dopp1}:$$\nu_R=\nu_S/\gamma$$
    \end{itemize}
  \end{corollary}
