...
Consider two objects of masses $m(t)$ and $dm$ and velocities $\textbf{v}(t)$ and $\textbf{u}(t)$ respectively, which in an interval of time $dt$ the second one collide with the first one and become a unique object. If $\textbf{F}^\text{ext}$ is the external force acting to the system, we have 
\begin{equation}
    \textbf{F}^\text{ext}=m\dot{\textbf{v}}+(\textbf{v}-\textbf{u})\dot{m}=\dot{\textbf{p}}-\dot{m}\textbf{u},
    \label{mass}
\end{equation} where $\dot{\textbf{p}}$ is the momentum of the object that gains mass.\footnote{The formula is also valid for the case when the object is losing mass, i.e. $\dot{m}<0$.}
\end{concept}
...
The phase space of the simple harmonic oscillator is $$\textbf{u}=\begin{pmatrix}
x(t)\\
\dot{x}(t)
...
\begin{itemize}
    ...
    \begin{figure}[ht]
        \centering
        \captionbox{Mass hanging from a spring.}{    \resizebox{.6\linewidth}{!}{\subimport{Images/}{springs.tex}}}
    \end{figure} 
    \item Simple pendulum: $$\theta(t)=A\cos(\omega_0t+\phi),\quad\omega_0=\sqrt{\frac{g}{l}}.$$
    \begin{figure}[ht]
        \centering
        \captionbox{Simple pendulum.}{    \resizebox{.6\linewidth}{!}{\subimport{Images/}{simple_pendulum.tex}}}
    \end{figure} 
    \item Physical pendulum: 
    $$\theta(t)=A\cos(\omega_0t+\phi),\quad\omega_0=\sqrt{\frac{mgD}{I_e}}.$$
    \begin{figure}[ht] 
        \centering 
        \captionbox{Physical pendulum.}{    \resizebox{.6\linewidth}{!}{\subimport{Images/}{physical_pendulum.tex}}}
    \end{figure} 
