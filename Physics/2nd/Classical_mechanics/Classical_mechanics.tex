...
\begin{multicols}{2}[\section{Classical mechanics}]
  ...
  \begin{prop}[Rocket with gravity]
    In this case, and using the hypothesis of \cref{CM_rocket}, we have $\vf{F}^\text{ext}=-mg\vf{j}$. Suppose $\vf{v}=v\vf{j}$, $\vf{c}=-c\vf{j}$ and, for simplicity, consider only the case when $\dot{m}=-\beta$, $\beta>0$. Therefore, we obtain:
    \begin{equation}
  ...
  \begin{prop}
    ...
    \begin{equation}
      ...
      \label{CM_mhs}
    \end{equation} where $\displaystyle A=\sqrt{{x_0}^2+{\left(\frac{\dot{x}_0}{\omega_0}\right)}^2}$ and $\displaystyle \phi=-\arctan\frac{\dot{x}_0}{\omega_0x_0}$. Such constants $\omega_0$ ($[\omega_0]=\text{rad}\cdot \text{s}^{-1}$), $A$ ($[A]=\text{m}$) and $\phi$ ($[\phi]=\text{rad}$) are called \emph{angular frequency}, \emph{amplitude} and \emph{initial phase}, respectively. Observe that the function in \cref{CM_mhs} is periodic with period $T=\frac{2\pi}{\omega_0}$ ($[T]=\text{s}$) and frequency $\nu=T^{-1}=\frac{\omega_0}{2\pi}$ ($[\nu]=\text{s}^{-1}=:\text{Hz}$).
  \end{prop}
  \begin{prop}
  ...
  \end{definition}
  \begin{prop}
    Let $U(x)$ be a potential function of class $\mathcal{C}^2(\RR)$ associated with a particle of mass $m$ and $x_0$ be a point of stable equilibrium. Then, if we disturb slightly the particle, it will start to oscillate at a frequency $\omega_0=\sqrt{\frac{U''(x_0)}{m}}$.
  \end{prop}
  \begin{prop}
  ...
  \end{prop}
  \begin{definition}
    Given a driven oscillator, we say it is in the \emph{steady-state part} if $t\gg 1/\beta$. In that case $x(t)$ become: $$x(t)=A\cos(\omega t+\psi-\phi).$$ While the dependency on $c_1,c_2$ is non-negligible, we say the driven oscillator is in the \emph{transient part}.
  \end{definition}
  \begin{prop}[Resonance in amplitude]
    If $\omega=\omega_r:=\sqrt{\omega_0^2-2\beta^2}$ we say the oscillator is in \emph{resonance in amplitude}. For $\omega=\omega_r$ we have $$A_r=\frac{f_0}{2\beta\sqrt{\omega_0^2-\beta^2}}.$$
  \end{prop}
  ...
  \end{prop}
  \begin{definition}
    We define the \emph{cutoff frequencies} as this two frequencies: $$\omega_1=\omega_0-\beta,\quad\omega_2=\omega_0+\beta.$$ The value  $\Delta\omega=\omega_2-\omega_1=2\beta$ is called the \emph{bandwidth}. Therefore, we can redefined the quality factor as: $$Q=\frac{\omega_0}{2\beta}=\frac{\omega_0}{\Delta\omega}=\frac{\nu_0}{\Delta\nu}$$
  \end{definition}
  ...
  \begin{definition}[Central force]
    A \emph{central force} is a force of the form $$\vf{F}(\boldsymbol{r})=f(r)\boldsymbol{e}_r,$$ where $r=\|\boldsymbol{r}\|$ and $\boldsymbol{e}_r=\boldsymbol{r}/r$ is the unit radial vector.
  \end{definition}
  \begin{definition}
    The origin $\boldsymbol{r}=0$ is called \emph{center of forces}.
  \end{definition}
  \begin{prop}
  ...
  \end{prop}
  \begin{prop}[Orbits]
    The minimum of $U_\text{eff}(r)$ determines the radius of the \emph{stable circular orbits}. If $E<0$, the orbits are in a range of radius $r_\text{min}$ and $r_\text{max}$, and so are \emph{bounded orbits}. If $E>0$, the orbits are called \emph{unbounded orbits}. Equating \cref{CM_energy} to ??????, we can obtain the angle $\Delta\theta$ in a \emph{radial oscillation}:\footnote{A radial oscillation is the trajectory when $r$ moves from $r_\text{min}$ to $r_\text{max}$ and the comes back to $r_\text{min}$.} $$\Delta\theta=2\int_{r_\text{min}}^{r_\text{max}}\frac{l/r^2}{\sqrt{(2/m)(E-U_\text{eff}}}dr.\footnote{Here we have taken the positive orientation, that is, $\boldsymbol{L}$ pointing to the positive $z$-axis.}$$ Note, that the orbits are \emph{closed orbits} if $$\Delta\theta=2\pi\frac{p}{q},\quad p,q\in\NN.$$
  \end{prop}
  ...
  \end{prop}
  \begin{definition}
    We define the \emph{pericenter of an orbit} as the minimum value of $r(\theta)$, that is, $r=r_\text{min}$. Analogously, we define the \emph{apocenter of an orbit} as the maximum value of $r(\theta)$, that is, $r=r_\text{max}$.
  \end{definition}
