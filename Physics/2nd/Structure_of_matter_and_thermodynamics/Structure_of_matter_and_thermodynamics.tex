...
\begin{multicols}{2}[\section{Structure of matter and thermodynamics}]
  ...
  \begin{proposition}[Particle in a box-1D]
    Consider a particle of mass $m$ confined in a 1-dimensional box of length $L$. We can think that it is subjected to a potential $$U(x)=
      ...
      \end{cases}
    $$
    Since the particle cannot leave the region $(0,L)$, we have that $\Phi(x)=0$ for $x\leq 0$ and $x\geq L$. Therefore, by \mcref{SMT_schrodinger}:
    $$
  ...
  \begin{definition}[Quantum numbers in spherical coordinates]
    ...
    \begin{itemize}
      \item \emph{Principal quantum number}: $n=1,2,3,\ldots$\par
            The energy, as seen in \mcref{SMT_energy-levels}, is given by: $$E_n=-Z^2\frac{13.6\text{ eV}}{n^2}$$
...
\begin{multicols}{2}
  ...
  \begin{definition}[Carnot cylce]
    ...
    where $T_\text{h}$ and $T_\text{c}$ are the temperatures at the isothermal processes (see \mcref{carnot}). Moreover it is satisfied that: $$\frac{Q_\text{c}}{Q_\text{h}}=\frac{T_\text{c}}{T_\text{h}}$$
    \begin{center}
      \begin{minipage}{\linewidth}
        \centering
